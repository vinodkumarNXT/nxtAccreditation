...
@Component({
  selector: 'lib-student-strength-add',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './student-strength-add.component.html',
  styleUrl: './student-strength-add.component.css',
})
export class StudentStrengthAddComponent {
  ...
  form: FormGroup;
  ugStudentForm: FormGroup;
  pgStudentForm: FormGroup;

  formType: 'UG' | 'PG' = 'UG'; // You can dynamically change this value based on user selection

  // Define common form controls for UG and PG
  formFields = {
    UG: [
      { name: 'maleStd', label: 'Male Students' },
      { name: 'femaleStd', label: 'Female Students' },
      { name: 'totalStd', label: 'Total Students' },
      { name: 'withinState', label: 'Within State' },
      { name: 'outsideState', label: 'Outside State' },
      { name: 'outsideCountry', label: 'Outside Country' },
      { name: 'economicallyBackward', label: 'Economically Backward' },
      { name: 'sociallyChallenged', label: 'Socially Challenged' },
      {
        name: 'stateCentralFeeReimbursement',
        label: 'State & Central Fee Reimbursement',
      },
      {
        name: 'institutionFundsFeeReimbursement',
        label: 'Institution Funds Fee Reimbursement',
      },
      {
        name: 'privateBodiesFeeReimbursement',
        label: 'Private Bodies Fee Reimbursement',
      },
      { name: 'noFeeReimbursement', label: 'No Fee Reimbursement' },
    ],
    PG: [
      { name: 'maleStd', label: 'Male Students' },
      { name: 'femaleStd', label: 'Female Students' },
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      dynamicYears: this.fb.array([]), // FormArray to hold dynamic year sets
    });

    // Add an initial set of fields
    this.initializeForms();
  }

 // Initialize the forms
 initializeForms() {
  // UG Form Initialization
  const ugControls = this.formFields.UG.reduce((acc, field) => {
    acc[field.name] = ['', Validators.required]; // Add validators if needed
    return acc;
  }, {});

  this.ugStudentForm = this.fb.group(ugControls);

  // PG Form Initialization
  const pgControls = this.formFields.PG.reduce((acc, field) => {
    acc[field.name] = ['', Validators.required]; // Add validators if needed
    return acc;
  }, {});

  this.pgStudentForm = this.fb.group(pgControls);
}


// Method to check if both forms are valid
isFormValid() {
  return this.ugStudentForm.valid && this.pgStudentForm.valid;
}


async submitForm(formType: any) {
  this.submitted = true;

  // Check if both forms are valid
  if (this.ugStudentForm.valid && this.pgStudentForm.valid) {

    try {
      // Collect form data for UG and PG
      const ugData = this.ugStudentForm.value;
      const pgData = this.pgStudentForm.value;

      // Combine the data from both forms into one object
      const combinedData = {
        UG: ugData,
        PG: pgData
      };
      ...
      // Call your form submission service
      const response = await this.formsService.onFormSubmit(combinedData, endpoints.NIRFconsultancyProjectDetails);

      // Show success toast notification
      this.swalService.addSuccess();

      // Refresh the list data after submission
      this.listData = response.data;
      this.dataAdded.emit(); // Notify the parent component
      this.resetForm();

      // Function to simulate a delay before hiding the spinner
    ...
    // Show warning toast notification if any form is invalid
  ...
  async loadFormData() {
    try {
      ...
      const listData = await this.formsService.getListData(
        endpoints.NIRFconsultancyProjectDetails
      );
  ...
  async loadMetaData() {
    try {
      ...
      const metaData = await this.formsService.getListData(
        endpoints.NIRFconsultancyProjectDetails
      );
  ...
  onFilesSelected(event: any): void {
    ...
    if (this.selectedFiles.length + files.length > 8) {
      ...
      this.swalService.warningNotification(
        'You can upload a maximum of 8 files.'
      );
      return;
    }
  ...
  async processFiles(
    ...
    for (const file of files) {
      ...
      if (file.size > maxSizeInBytes) {
        ...
        await this.swalService.warningNotification(
          `File size (${fileSizeMB} MB) exceeds the 2MB limit.`
        );
        continue;
      }
  ...
  simulateUploadProgress(
    progressInfo: { fileName: string; value: number },
    resolve: () => void
  ): void {
    let progress = 0;
