...
import { TablePaginationService, StudentStrengthAddComponent, StudentStrengthEditComponent, StudentStrengthViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-student-strength',
  standalone: true,
 imports: [MaterialModule, SharableModule],
  ...
  styleUrl: './student-strength.component.css'
})
export class StudentStrengthComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "maleStd", "feMaleStd", "totalStd", "withinState", "outsideState", "outsideCountry",
    //  "economicallyBackward","sociallyChallenged",
    // "stateCentralFeeReimbursement", "institutionFundsFeeReimbursement","privateBodiesFeeReimbursement","noFeeReimbursement", 
   "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.NIRFstudentStrength);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(StudentStrengthAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(StudentStrengthViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(StudentStrengthEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.NIRFstudentStrength;
