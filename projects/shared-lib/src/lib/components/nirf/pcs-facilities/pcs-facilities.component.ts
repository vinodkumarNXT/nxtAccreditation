...
import { PcsFacilitiesAddComponent, PcsFacilitiesEditComponent, PcsFacilitiesViewComponent, TablePaginationService, } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-pcs-facilities',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './pcs-facilities.component.html',
  styleUrl: './pcs-facilities.component.css'
})
export class PcsFacilitiesComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "liftAndRamp", "walkingAids", "handicappedToilet", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.NIRFIPR);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(PcsFacilitiesAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(PcsFacilitiesViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(PcsFacilitiesEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.NIRFIPR;
