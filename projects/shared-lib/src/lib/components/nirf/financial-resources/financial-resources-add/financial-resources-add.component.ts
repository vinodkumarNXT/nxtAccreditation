...
@Component({
  selector: 'lib-financial-resources-add',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './financial-resources-add.component.html',
  styleUrl: './financial-resources-add.component.css'
})
export class FinancialResourcesAddComponent {
   ...
   public enteredYear:any
    ...
    form: FormGroup;
    capitalExpenditureForm: FormGroup;
    operationalExpenditureForm: FormGroup;


    readonly startDate = new FormGroup({
      start: new FormControl(new Date(year, 7)),
      end: new FormControl(new Date(year, 7)),
    });

    readonly endDate = new FormGroup({
      start: new FormControl(new Date(year, 7)),
      end: new FormControl(new Date(year, 7)),
    });
      ...
      constructor(
        ...
        this.capitalExpenditureForm = this.fb.group({
          expenditures: this.fb.array([this.createExpenditureControl()]),
        });

        this.operationalExpenditureForm = this.fb.group({
          OpreationalExpenditures: this.fb.array([this.createExpenditureControl()]),
        });
      }
    

      get expenditures(): FormArray {
        return this.capitalExpenditureForm.get('expenditures') as FormArray;
      }

      get operationalExpenditures(): FormArray {
        return this.capitalExpenditureForm.get('opertaionalExpenditures') as FormArray;
      }
    
      createExpenditureControl(): FormGroup {
   ...
   createOperationExpenditures(): FormGroup {
        return this.fb.group({
          opertionalDescription: ['', Validators.required],
          opertionalAmount: [null, [Validators.required, Validators.min(0)]],
        });
      }
    
      addExpenditure(event:any): void {
        this.expenditures.push(this.createExpenditureControl());
      }
    
      removeExpenditure(index: number): void {
        this.expenditures.removeAt(index);
      }


      addOperationalExpenditure(event:any): void {
        this.expenditures.push(this.createExpenditureControl());
      }
    
      removeOperationalExpenditure(index: number): void {
        this.expenditures.removeAt(index);
      }
  ...
  // Getter for dynamicYears FormArray
  get dynamicYears(): FormArray {
    return this.form.get('dynamicYears') as FormArray;
  }
  ...
  addYearSet(): void {
    const yearGroup = this.fb.group({
      ...
      pgEnrolled: ['', Validators.required],
      phdIntake: ['', Validators.required],
      phdEnrolled: ['', Validators.required],
    });
