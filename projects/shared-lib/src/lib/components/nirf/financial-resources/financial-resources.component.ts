...
import { TablePaginationService, ConsultancyProjectAddComponent, ConsultancyProjectViewComponent, ConsultancyProjectEditComponent, FinancialResourcesAddComponent, FinancialResourcesViewComponent, FinancialResourcesEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-financial-resources',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './financial-resources.component.html',
  styleUrl: './financial-resources.component.css'
})
export class FinancialResourcesComponent {
 
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year", "utilisedAmount", "capitalExpenditure", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.NIRFfinancialResources);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(FinancialResourcesAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(FinancialResourcesViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(FinancialResourcesEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.NIRFfinancialResources;
