...
import { IprAddComponent, IprEditComponent, IprViewComponent, TablePaginationService, } from 'shared-lib';
...
@Component({
  ...
  styleUrl: './ipr.component.css'
})
export class IprComponent {
 
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year", "patentsPublished", "patentsGranted", "actions"];
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(IprAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(IprViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(IprEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
