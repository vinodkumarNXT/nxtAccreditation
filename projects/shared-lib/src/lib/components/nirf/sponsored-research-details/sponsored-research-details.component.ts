import { SponsoredResearchDetailsAddComponent } from './sponsored-research-details-add/sponsored-research-details-add.component';
...
import { TablePaginationService, IprAddComponent, IprViewComponent, IprEditComponent, SponsoredResearchDetailsViewComponent, SponsoredResearchDetailsEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-sponsored-research-details',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './sponsored-research-details.component.html',
  styleUrl: './sponsored-research-details.component.css'
})
export class SponsoredResearchDetailsComponent {
 
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year", "noOfSponsoredProjects", "noofFundingAgencis","totalrxAmount", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.NIRFsponsoredResearchDetails);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(SponsoredResearchDetailsAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(SponsoredResearchDetailsViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(SponsoredResearchDetailsEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.NIRFsponsoredResearchDetails;
