...
import { TablePaginationService, ConsultancyProjectAddComponent, ConsultancyProjectViewComponent, ConsultancyProjectEditComponent, PlacementHigherStudiesAddComponent, PlacementHigherStudiesViewComponent, PlacementHigherStudiesEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-placement-higher-studies',
  standalone: true,
  imports: [MaterialModule, SharableModule],
   providers: [FormsService, provideNativeDateAdapter()],
   changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './placement-higher-studies.component.html',
  styleUrl: './placement-higher-studies.component.css'
})
export class PlacementHigherStudiesComponent {
 ...
 pageSizeOptions: number[] = [];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.NIRFplacementHigherStudies);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(PlacementHigherStudiesAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(PlacementHigherStudiesViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(PlacementHigherStudiesEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.NIRFplacementHigherStudies;
