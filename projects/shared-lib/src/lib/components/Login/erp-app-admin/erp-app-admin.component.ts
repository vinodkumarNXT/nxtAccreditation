...
import { ErpAppAdminAddComponent, ErpAppAdminEditComponent, ErpAppAdminViewComponent, FormsService, MaterialModule, SharableModule, SwalService } from 'school-erp-public';
import { FileUploadService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureEditComponent, InfrastractureExpenditureViewComponent, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-erp-app-admin',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './erp-app-admin.component.html',
  styleUrl: './erp-app-admin.component.css'
})


export class ErpAppAdminComponent {



  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "username","domain", "branch","hosting","affiliation","contactNumber","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolERPAppAdminRegister);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ErpAppAdminAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ErpAppAdminViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ErpAppAdminEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolERPAppAdminRegister;
