...
import { MaterialModule, SharableModule, FormsService, SwalService, FloorEditComponent, FloorViewComponent, FloorAddComponent } from 'school-erp-public';
...
export class FloorComponent {
  ...
  displayedColumns = ["id", "FloorName","buildingName", "status","actions"];
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(FloorAddComponent, {});
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolFloor;
