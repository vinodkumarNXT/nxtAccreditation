...
import { UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';

@Component({
  selector: 'lib-building-add',
  standalone: true,
  ...
  styleUrl: './building-add.component.css'
})
    ...
    form: FormGroup;
      ...
      async submitForm() {
        ...
        if (this.form.valid) {
          ...
          try {
            const response = await this.formsService.onFormSubmit(postData, schoolErpEndpoint.SchoolBuildings);
