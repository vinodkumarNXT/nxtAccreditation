...
import { EnvironmentService } from 'projects/shell/src/app/services/environment.service';
import { Subject } from 'rxjs';
import { MaterialModule, SharableModule, FormsService, SwalService, BuildingEditComponent, BuildingViewComponent, BuildingAddComponent, SessionService } from 'school-erp-public';
import { TablePaginationService } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-building',
  standalone: true,
...
export class BuildingComponent {
  ...
  displayedColumns = ["id", "name", "branch_code", "status", "actions"];
  ...
  constructor(
    ...
    // Retrieve the branch code when the component initializes
    this.branchID = this.sessionService.getbranchID();
    console.log('Current Branch Code:', this.branchID);
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolBuildings;
