...
import { MaterialModule, SharableModule, FormsService, SwalService, RoomEditComponent, RoomAddComponent } from 'school-erp-public';
...
export class RoomComponent {
  ...
  displayedColumns = ["id", "buildingName","floorName","roomName","roomCapacity","status", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolRooms);
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolRooms;
