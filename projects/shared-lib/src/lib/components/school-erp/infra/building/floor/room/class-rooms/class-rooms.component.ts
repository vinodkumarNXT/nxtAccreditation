...
import { UnderscoreToSpacePipe, YearMonthDayPipe, FileUploadService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureEditComponent, InfrastractureExpenditureViewComponent, TablePaginationService } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-class-rooms',
  standalone: true,
  ...
  styleUrl: './class-rooms.component.css'
})
export class ClassRoomsComponent {
  ...
  displayedColumns = ["id","classRoomName", "Status", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolClassRooms);
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolClassRooms;
