...
import { MaterialModule, SharableModule, FormsService, SwalService, GroundAddComponent, GroundViewComponent, GroundEditComponent } from 'school-erp-public';
...
export class GroundComponent {
  ...
  displayedColumns = ["id","name", "typeOfGround","address",'status', "actions"];
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(GroundViewComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolSportsGround;
