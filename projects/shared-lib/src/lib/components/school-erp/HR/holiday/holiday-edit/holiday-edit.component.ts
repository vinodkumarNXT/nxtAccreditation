...
export class HolidayEditComponent {
  ...
  async loadMetaData() {
    try {
      ...
      const metaData = await this.formsService.getListData(schoolErpEndpoint.SchoolHolidayMaster);
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, schoolErpEndpoint.SchoolHolidayMaster);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, schoolErpEndpoint.SchoolHolidayMaster);
