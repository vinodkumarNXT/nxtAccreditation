...
import { MaterialModule, SharableModule, FormsService, SwalService, HolidayAddComponent, HolidayEditComponent, HolidayViewComponent } from 'school-erp-public';
...
export class HolidayComponent {
  ...
  displayedColumns = ["id", "title", "description", "date","actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolHolidayMaster);
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(HolidayViewComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolHolidayMaster;
