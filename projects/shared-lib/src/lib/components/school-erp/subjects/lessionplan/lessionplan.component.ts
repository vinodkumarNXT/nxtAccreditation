...
@Component({
  selector: 'lib-lessionplan',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, ConfirmationService, MessageService,DialogService],
  templateUrl: './lessionplan.component.html',
  styleUrl: './lessionplan.component.css'
})
export class LessionplanComponent {
  ...
  async openEditDialog(event?:any){
    ...
    try {
      const response = await this.formsService.getFormData(selectedID, schoolErpEndpoint.SchoolCreateLessonPlan);
  ...
  async loadTableData() {
    try {
      ...
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolCreateLessonPlan);
