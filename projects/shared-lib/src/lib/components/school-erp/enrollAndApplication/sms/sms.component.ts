...
import { MaterialModule, SharableModule, FormsService, SwalService, SmsAddComponent, SmsViewComponent, SmsEditComponent } from 'school-erp-public';
...
export class SmsComponent {
  ...
  displayedColumns = ["id", "driver","sid", "nxemoKey","twillioToken","nxemoSecret","nxemoName","twillioNumber","actions"];
...
//      "sid":"fsdfdgfdfrg",
//      "nxemoKey":"7e29c3ce",
//      "twillioToken":"fsdfsdgfv",
//      "nxemoSecret":"7e29c3ce",
//       "nxemoName":"test",
//       "twillioNumber":1234567890,
//     "status":2
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolSMS);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(SmsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(SmsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(SmsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolSMS;
