...
export class BranchAddComponent {
    ...
    ngOnInit(): void {
     ...
     async submitForm() {
      this.submitted = true; // Track the submission status
      ...
      if (this.form.valid) {
          ...
          try {
              ...
              // Refresh the list data with the new response
              this.listData = response.data;
  
              // Notify the parent component about the new data
              this.dataAdded.emit();
              ...
              await this.delay(1000); // Hold the spinner for an additional 2 seconds
  
              // Close all dialogs after completion
              this.matDialog.closeAll();
          } catch (error) {
              console.error("Error submitting form:", error); // Log the error for debugging
              this.swalService.addError();
          } finally {
              // Ensure the spinner is hidden regardless of success or failure
              this.loading = false;
          }
      } else {
          // Show warning toast notification for invalid form
          await this.swalService.warningNotification('Invalid Form', 'Please check the fields and try again.');
          this.loading = false; // Ensure the spinner is hidden if the form is invalid
      }
  }
   ...
   updateForm() {
    ...
    async loadFormData() {
      try {
        ...
        this.getMetaData = await this.formsService.getListData(schoolErpEndpoint.SchoolMetaData);
