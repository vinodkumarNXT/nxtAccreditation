...
import { MaterialModule, SharableModule, FormsService, SwalService, LanguagesAddComponent, LanguagesViewComponent, LanguagesEditComponent } from 'school-erp-public';
...
export class LanguagesComponent {
  ...
  displayedColumns = ["id", "name","code", "direction","status","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolLanguage);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(LanguagesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(LanguagesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(LanguagesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolLanguage;
