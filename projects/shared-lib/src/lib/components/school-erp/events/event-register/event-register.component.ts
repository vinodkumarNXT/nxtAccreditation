...
import { MaterialModule, SharableModule, FormsService, SessionService, SwalService, EventRegisterAddComponent, EventTypeEditComponent, EventTypeViewComponent } from 'school-erp-public';
import { FileUploadService, TablePaginationService } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-event-register',
  standalone: true,
...
export class EventRegisterComponent {
  ...
  displayedColumns = ["id", "studentID","studentName","eventName","startDate","endDate", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolEventRegister);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(EventRegisterAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(EventTypeViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(EventTypeEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolEventRegister;
