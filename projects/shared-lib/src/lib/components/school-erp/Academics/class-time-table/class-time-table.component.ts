...
import { MaterialModule, SharableModule, FormsService, SessionService, SwalService, ClassTimeTableAddComponent, ClassTimeTableViewComponent, ClassTimeTableEditComponent } from 'school-erp-public';
import { FileUploadService, TablePaginationService } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-class-time-table',
  standalone: true,
  ...
  styleUrl: './class-time-table.component.css'
})
export class ClassTimeTableComponent implements OnInit {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", 'dayOfWeek','status',"actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(schoolErpEndpoint.SchoolClassTimeTables);
  ...
  openAddDialog(): void {
    ...
    const url = this.router.serializeUrl(
      this.router.createUrlTree(['/class-time-table-add']) // The route to the component
    );
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ClassTimeTableViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ClassTimeTableEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = schoolErpEndpoint.SchoolClassTimeTables;
