...
import { MaterialModule, SharableModule, FormsService, SessionService, SwalService, passwordValidator } from 'school-erp-public';

@Component({
  selector: 'lib-erp-master-admin-register',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './erp-master-admin-register.component.html',
  styleUrl: './erp-master-admin-register.component.css'
})
export class ErpMasterAdminRegisterComponent {
  ...
  secretKey: string = '';
    ...
    async fetch2faQRCode() {
      ...
      try {
        // Fetch the metadata from the server (e.g., the QR code and secret key)
        const metaData = await this.formsService.register2fa(schoolErpEndpoint.SchoolERPMasterAdminRegister);
        
        // Check if metaData has the required structure
        if (metaData && metaData.data) {
          // Store the QR code URL and secret key from the response
          this.getQRMetaData = metaData.data;
          this.qrCodeUrl = this.getQRMetaData.qrCodeUrl; // Store the QR code URL
          this.secretKey = this.getQRMetaData.secretKey; // Store the secret key for later use
        } else {
          throw new Error('Failed to fetch 2FA metadata.');
        }
        
      } catch (error) {
        console.error('Error fetching 2FA QR code:', error);
        // Handle the error appropriately (e.g., show an error message)
        // For now, you can add a toast or any error handling mechanism
        this.qrCodeUrl = ''; // Reset the QR code URL in case of error
        this.secretKey = ''; // Reset the secret key in case of error
        // alert('Failed to fetch 2FA QR code. Please try again.');
        this.swalService.warningNotification('Failed to fetch 2FA metadata.')

        
      } finally {
        this.loading = false; // Stop loading indicator once the process is complete
      }
    }
    


     onBuildingSelection(event: MatSelectChange) {
      const selectedId = event.value;
      this.selectedBuildingId = selectedId;
    }
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, schoolErpEndpoint.SchoolFloor);
