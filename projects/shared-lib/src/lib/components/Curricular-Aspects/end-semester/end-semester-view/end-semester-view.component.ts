...
import { MaterialModule, RandomColorService, FormsService, SharableModule } from 'school-erp-public';
import { PercentageofProgramsEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-end-semester-view',
  imports: [MaterialModule, SharableModule],
  standalone:true,
  templateUrl: './end-semester-view.component.html',
  styleUrl: './end-semester-view.component.css'
})
export class EndSemesterViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.EndSemester);
