...
@Component({
  selector: 'lib-end-semester-edit',
  standalone: true,
  ...
  templateUrl: './end-semester-edit.component.html',
  styleUrl: './end-semester-edit.component.css'
})
export class EndSemesterEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        ...
        semester_year: this.form.get("semesterYear").value,
        examEndDate: this.form.get("examEndDate").value,
        resultDate: this.form.get("resultDate").value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.EndSemester);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.EndSemester);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          ...
          semesterYear: formData?.semester_year,
          resultDate: formData?.result_date,
          examEndDate:formData?.exam_end_date,
