...
@Component({
  selector: 'lib-competitions-organized-edit',
  standalone: true,
  ...
  templateUrl: './competitions-organized-edit.component.html',
  styleUrl: './competitions-organized-edit.component.css'
})
export class CompetitionsOrganizedEditComponent {
  ...
  public isSubmittedeFn:boolean= true;
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.CompetitionsOrganized);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.CompetitionsOrganized);
