...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureViewComponent, InfrastractureExpenditureEditComponent, CompetitionsOrganizedAddComponent, CompetitionsOrganizedViewComponent, CompetitionsOrganizedEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-competitions-organized',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './competitions-organized.component.html',
  styleUrl: './competitions-organized.component.css'
})
export class CompetitionsOrganizedComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year","eventDate", "eventName","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.CompetitionsOrganized);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(CompetitionsOrganizedAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(CompetitionsOrganizedViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(CompetitionsOrganizedEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.CompetitionsOrganized;
