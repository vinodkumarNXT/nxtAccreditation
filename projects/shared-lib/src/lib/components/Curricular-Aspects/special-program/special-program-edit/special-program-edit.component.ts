...
@Component({
  selector: 'lib-special-program-edit',
  standalone: true,
  ...
  templateUrl: './special-program-edit.component.html',
  styleUrl: './special-program-edit.component.css'
})
export class SpecialProgramEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.SpecialProgram);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.SpecialProgram);
