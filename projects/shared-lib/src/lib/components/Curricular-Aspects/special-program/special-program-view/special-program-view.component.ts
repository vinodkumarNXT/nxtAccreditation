...
@Component({
  selector: 'lib-special-program-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './special-program-view.component.html',
  styleUrl: './special-program-view.component.css'
})
export class SpecialProgramViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.SpecialProgram);
