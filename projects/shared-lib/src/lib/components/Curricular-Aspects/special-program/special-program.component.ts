...
export class SpecialProgramComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "programName", "actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.SpecialProgram);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(SpecialProgramAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(SpecialProgramViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(SpecialProgramEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.SpecialProgram;
