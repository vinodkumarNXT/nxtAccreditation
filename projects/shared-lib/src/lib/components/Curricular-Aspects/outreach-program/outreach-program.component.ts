...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService,  OutreachProgramEditComponent, OutreachProgramViewComponent, OutreachProgramAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-outreach-program',
  standalone: true,
  ...
  templateUrl: './outreach-program.component.html',
  styleUrl: './outreach-program.component.css'
})
export class OutreachProgramComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "activityName", "organizingUnit", "schemeName","activityYear", "participants", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.OutreachProgram);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(OutreachProgramAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(OutreachProgramViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(OutreachProgramEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.OutreachProgram;
