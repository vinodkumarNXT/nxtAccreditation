...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ICTFacilitiesAddComponent, ICTFacilitiesViewComponent, ICTFacilitiesEditComponent, ITFacilitiesEditComponent, ITFacilitiesViewComponent, ITFacilitiesAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-it-facilities',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './it-facilities.component.html',
  styleUrl: './it-facilities.component.css'
})
export class ITFacilitiesComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "itFacility","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ITFacilities);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ITFacilitiesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ITFacilitiesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ITFacilitiesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ITFacilities;
