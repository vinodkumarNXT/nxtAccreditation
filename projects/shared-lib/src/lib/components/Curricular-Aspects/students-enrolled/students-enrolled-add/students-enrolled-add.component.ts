...
@Component({
  selector: 'lib-students-enrolled-add',
  standalone: true,
  ...
  templateUrl: './students-enrolled-add.component.html',
  styleUrl: './students-enrolled-add.component.css'
})
export class StudentsEnrolledAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.StudentsEnrolled);
