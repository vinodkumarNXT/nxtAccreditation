...
export class StudentsEnrolledComponent implements OnInit , AfterViewInit  {
...
//     "upload_files": [
//         "1725016712_Screenshot_(8).png",
//         "1725016712_Screenshot_(9).png"
//     ],
//     "document_links": null
// }

  displayedColumns = ["id", "courseCode", "courseName", 'offeringYear','courseDuration', 'enrolledStudents', 'actions'];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.StudentsEnrolled);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(StudentsEnrolledAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(StudentsEnrolledViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(StudentsEnrolledEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.StudentsEnrolled;
