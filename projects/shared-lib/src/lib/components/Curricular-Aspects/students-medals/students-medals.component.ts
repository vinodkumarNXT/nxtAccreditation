...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureViewComponent, InfrastractureExpenditureEditComponent, StudentsMedalsAddComponent, StudentsMedalsViewComponent, StudentsMedalsEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-students-medals',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './students-medals.component.html',
  styleUrl: './students-medals.component.css'
})
export class StudentsMedalsComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year","awardName", "level","eventName","studentName","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.StudentsMedals);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(StudentsMedalsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(StudentsMedalsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(StudentsMedalsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.StudentsMedals;
