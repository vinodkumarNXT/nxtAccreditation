...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, CitationsAddComponent, CitationsViewComponent, CitationsEditComponent, ConsultancyRevenueAddComponent, ConsultancyRevenueEditComponent, ConsultancyRevenueViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-consultancy-revenue',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './consultancy-revenue.component.html',
  styleUrl: './consultancy-revenue.component.css'
})
export class ConsultancyRevenueComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "staffName", "departmentName", "agencyName","year", "consultancyName", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ConsultancyRevenue);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ConsultancyRevenueAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ConsultancyRevenueViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ConsultancyRevenueEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ConsultancyRevenue;
