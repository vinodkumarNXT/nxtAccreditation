...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService,  AdministrativeGovernanceAddComponent, AdministrativeGovernanceViewComponent, AdministrativeGovernanceEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-administrative-governance',
  standalone: true,
  ...
  templateUrl: './administrative-governance.component.html',
  styleUrl: './administrative-governance.component.css'
})
export class AdministrativeGovernanceComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "VisionAndMission","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AdministrativeGovernance);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(AdministrativeGovernanceAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(AdministrativeGovernanceViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(AdministrativeGovernanceEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.AdministrativeGovernance;
