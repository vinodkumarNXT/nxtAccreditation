...
@Component({
  selector: 'lib-student-grievances-add',
  standalone: true,
  ...
  templateUrl: './student-grievances-add.component.html',
  styleUrl: './student-grievances-add.component.css'
})
export class StudentGrievancesAddComponent {
    ...
    constructor(
      ...
      this.form = this.formBuilder.group({
        ...
        guidanceActivity: ['', Validators.required],
        campusPlacement: ['', Validators.required],
        counsellingActivity: ['', Validators.required],
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        const postData = {
          ...
          guidanceActivity: this.form.get("guidanceActivity")?.value,
          campusPlacement: this.form.get("campusPlacement")?.value,
          counsellingActivity: this.form.get("counsellingActivity")?.value,
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, endpoints.StudentGrievances);
    ...
    async loadFormData() {
      try {
        ...
        const listData = await this.formsService.getListData(endpoints.StudentGrievances);
