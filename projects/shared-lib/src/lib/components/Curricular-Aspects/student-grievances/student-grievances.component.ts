...
export class StudentGrievancesComponent {


  pageSizeOptions: number[] = [];

  displayedColumns = ["id", "Year","guidanceActivity", "campusPlacement","counsellingActivity","participants",'actions'];
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(StudentGrievancesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(StudentGrievancesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(StudentGrievancesEditComponent, {
      // Pass the element's id as data to the dialog
      data: { id: val.id }
    });
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.StudentGrievances;
