...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognizationsReceivedAddComponent, RecognizationsReceivedViewComponent, RecognizationsReceivedEditComponent, EditedVolumesEditComponent, EditedVolumesViewComponent, EditedVolumesAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-edited-volumes',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './edited-volumes.component.html',
  styleUrl: './edited-volumes.component.css'
})
export class EditedVolumesComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "bookTitle", "publisherName","ISSNISBN", "publicationYear", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.EditedVolumes);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(EditedVolumesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(EditedVolumesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(EditedVolumesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.EditedVolumes;
