...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognizationsReceivedAddComponent, RecognizationsReceivedViewComponent, RecognizationsReceivedEditComponent, TeacherpaperResearchViewComponent, TeacherpaperResearchAddComponent, TeacherResearchEditComponent, TeacherpaperResearchEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-teacherpaper-research',
  standalone: true,
  ...
  templateUrl: './teacherpaper-research.component.html',
  styleUrl: './teacherpaper-research.component.css'
})
export class TeacherpaperResearchComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "paperTitle", "authorName", "departmentName","journalName","publicationYear", "actions"];
//   {
//     "id": 1,
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.TeacherResearchPapers);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(TeacherpaperResearchAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(TeacherpaperResearchViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(TeacherpaperResearchEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.TeacherResearchPapers;
