...
@Component({
  selector: 'lib-teacherpaper-research-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  
  templateUrl: './teacherpaper-research-add.component.html',
  styleUrl: './teacherpaper-research-add.component.css'
})
export class TeacherpaperResearchAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.TeacherResearchPapers);
