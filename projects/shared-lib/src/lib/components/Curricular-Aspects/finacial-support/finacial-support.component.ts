...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService,  FinacialSupportAddComponent, FinacialAuditViewComponent, FinacialAuditEditComponent, FinacialSupportEditComponent, FinacialSupportViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-finacial-support',
  standalone: true,
  ...
  templateUrl: './finacial-support.component.html',
  styleUrl: './finacial-support.component.css'
})
export class FinacialSupportComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year","teacherName", "workshop","body","amount","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.TeacherFinacialSupport);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(FinacialSupportAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(FinacialSupportViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(FinacialSupportEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.TeacherFinacialSupport;
