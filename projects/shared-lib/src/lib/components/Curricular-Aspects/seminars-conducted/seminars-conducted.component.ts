...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ResearchProjectsAddComponent, ResearchProjectsViewComponent, ResearchProjectsEditComponent, SeminarsConductedEditComponent, SeminarsConductedViewComponent, SeminarsConductedAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-seminars-conducted',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './seminars-conducted.component.html',
  styleUrl: './seminars-conducted.component.css'
})
export class SeminarsConductedComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id",  "participants", "eventName", "year","date" ,"actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.SeminarsConducted);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(SeminarsConductedAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(SeminarsConductedViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(SeminarsConductedEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.SeminarsConducted;
