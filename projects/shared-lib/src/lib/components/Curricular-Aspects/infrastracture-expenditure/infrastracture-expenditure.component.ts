...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, CulturalActivityFacilityAddComponent, CulturalActivityFacilityViewComponent, CulturalActivityFacilityEditComponent, InfrastractureExpenditureAddComponent, InfrastractureExpenditureViewComponent, InfrastractureExpenditureEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-infrastracture-expenditure',
  standalone: true,
  ...
  templateUrl: './infrastracture-expenditure.component.html',
  styleUrl: './infrastracture-expenditure.component.css'
})
export class InfrastractureExpenditureComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "budjetAllocated","expenditure", "totalExpenditure","academicMaintenance","physicalMaintenance","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.InfrastractureExpenditure);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(InfrastractureExpenditureAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(InfrastractureExpenditureViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(InfrastractureExpenditureEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.InfrastractureExpenditure;
