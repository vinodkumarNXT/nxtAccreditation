...
@Component({
  selector: 'lib-graduated-students-edit',
  standalone: true,
  ...
  templateUrl: './graduated-students-edit.component.html',
  styleUrl: './graduated-students-edit.component.css'
})
export class GraduatedStudentsEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      outgoingStudentCount: ['', Validators.required],
      totalStudentCount: ['', Validators.required],
      documentLink: [''],
    });
  }
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        outgoingStudentCount: this.form.get("outgoingStudentCount")?.value,
        totalStudentCount: this.form.get("totalStudentCount")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.GraduatedStudents);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.GraduatedStudents);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          outgoingStudentCount: formData?.outgoingStudentCount || '',
          totalStudentCount: formData?.totalStudentCount || '',
