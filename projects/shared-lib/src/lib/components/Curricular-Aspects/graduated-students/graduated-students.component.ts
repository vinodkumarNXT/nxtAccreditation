...
@Component({
  ...
  templateUrl: './graduated-students.component.html',
  styleUrl: './graduated-students.component.css'
})
export class GraduatedStudentsComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "outgoingStudentCount","totalStudentCount","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.GraduatedStudents);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(GraduatedStudentsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(GraduatedStudentsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(GraduatedStudentsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.GraduatedStudents;
