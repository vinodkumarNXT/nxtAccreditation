...
export class IntegrationReformsComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "textForIntegration","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.IntegrationReforms);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(IntegrationReformsAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(IntegrationReformsViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(IntegrationReformsEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.IntegrationReforms;
