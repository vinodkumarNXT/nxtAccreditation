...
@Component({
  selector: 'lib-integration-reforms-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './integration-reforms-add.component.html',
  styleUrl: './integration-reforms-add.component.css'
})
export class IntegrationReformsAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.IntegrationReforms);
