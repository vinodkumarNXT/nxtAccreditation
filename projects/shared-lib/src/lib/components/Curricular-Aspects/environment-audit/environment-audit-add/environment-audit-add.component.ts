...
export class EnvironmentAuditAddComponent {
    ...
    form: FormGroup;
      ...
      constructor(
        ...
        this.form = this.formBuilder.group({
          environmentText: ['', Validators.required],
      ...
      async submitForm() {
        ...
        if (this.form.valid) {
          ...
          const postData = {
            environmentText: this.form.get("environmentText")?.value,
            documentLink: this.form.get(" documentLink")?.value,
          };
    
          try {
            const response = await this.formsService.onFormSubmit(postData, endpoints.EnvironmentAudit);
     ...
     updateForm() {
      ...
      async loadMetaData() {
        try {
          ...
          const metaData = await this.formsService.getListData(endpoints.EnvironmentAudit);
