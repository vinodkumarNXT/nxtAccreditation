...
export class EnvironmentAuditComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "environmentText","upload_files","document_links","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.EnvironmentAudit);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(EnvironmentAuditAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(EnvironmentAuditViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(EnvironmentAuditEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.EnvironmentAudit;
