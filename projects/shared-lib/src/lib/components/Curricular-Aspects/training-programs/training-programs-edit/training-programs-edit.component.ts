...
@Component({
  selector: 'lib-training-programs-edit',
  standalone: true,
  ...
  templateUrl: './training-programs-edit.component.html',
  styleUrl: './training-programs-edit.component.css'
})

export class TrainingProgramsEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.TrainingPrograms);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.TrainingPrograms);
