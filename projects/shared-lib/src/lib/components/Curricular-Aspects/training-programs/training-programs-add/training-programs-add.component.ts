...
@Component({
  selector: 'lib-training-programs-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './training-programs-add.component.html',
  styleUrl: './training-programs-add.component.css'
})
export class TrainingProgramsAddComponent {
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, endpoints.TrainingPrograms);
