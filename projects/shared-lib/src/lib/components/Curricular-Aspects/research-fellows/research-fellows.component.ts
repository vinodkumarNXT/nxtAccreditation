...
import {FileUploadService, FormsService, ResearchFellowsAddComponent,
  ResearchFellowsEditComponent, ResearchFellowsViewComponent,
  SharableModule, SnackbarService,SwalService,TablePaginationService} from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-research-fellows',
  standalone: true,
  imports: [SharableModule],
  providers: [FormsService, SnackbarService, provideNativeDateAdapter()],
  templateUrl: './research-fellows.component.html',
  styleUrl: './research-fellows.component.css'
})
export class ResearchFellowsComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "researchFellowName", "enrollmentYear", "duration", "fellowshipType", "grantAgency","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ResearchFellow);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ResearchFellowsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ResearchFellowsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ResearchFellowsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ResearchFellow;
