...
@Component({
  selector: 'lib-e-governance-edit',
  standalone: true,
  ...
  templateUrl: './e-governance-edit.component.html',
  styleUrl: './e-governance-edit.component.css'
})

export class EGovernanceEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.EGovernance);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.EGovernance);
