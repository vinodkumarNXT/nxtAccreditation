...
@Component({
  ...
  templateUrl: './e-governance.component.html',
  styleUrl: './e-governance.component.css'
})
export class EGovernanceComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "area","year", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.EGovernance);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(EGovernanceAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(EGovernanceViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(EGovernanceEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.EGovernance;
