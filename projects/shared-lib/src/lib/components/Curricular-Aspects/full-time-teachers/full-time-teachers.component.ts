...
import { FileUploadService, FormsService, FullTimeTeachersAddComponent, FullTimeTeachersEditComponent, FullTimeTeachersViewComponent, MaterialModule, ReservedCategoryAddComponent, ReservedCategoryEditComponent, ReservedCategoryViewComponent, SharableModule, SwalService, TablePaginationService,  YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-full-time-teachers',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './full-time-teachers.component.html',
  styleUrl: './full-time-teachers.component.css'
})
export class FullTimeTeachersComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "totalStudents", "fullTimeTeachers",  "actions"];

//   {
//     "id": 1,
//     "total_students": "50",
//     "full_time_teachers": "20",
//     "upload_files": [
//         "1725430399_Screenshot_(8).png",
//         "1725430399_Screenshot_(8).png"
//     ],
//     "document_links": null
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.FullTimeTeachers);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(FullTimeTeachersAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(FullTimeTeachersViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  // Open the dialog with the InstitutionIntegratesEditComponent
  const dialogRef = this.dialog.open(FullTimeTeachersEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
}
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.FullTimeTeachers;
