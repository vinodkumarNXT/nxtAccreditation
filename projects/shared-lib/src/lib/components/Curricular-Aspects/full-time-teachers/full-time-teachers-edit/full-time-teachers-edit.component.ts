...
@Component({
  selector: 'lib-full-time-teachers-edit',
  standalone: true,
  ...
  templateUrl: './full-time-teachers-edit.component.html',
  styleUrl: './full-time-teachers-edit.component.css'
})
export class FullTimeTeachersEditComponent {
...
//   if (this.form.valid) {
//     const postData = {
//       totalStudents: this.form.get('totalStudents')?.value,
...
//     try {
//       // Attempt to update the form data using the service
//       await this.formsService.updateFormData(postData, this.editformID, endpoints.FullTimeTeachers);
...
async loadFormData(): Promise<void> {
  try {
    ...
    const formData = await this.formsService.getFormData(this.data.id, endpoints.FullTimeTeachers);
    ...
    this.form.patchValue({
      ...
      fullTimeTeachers:data?.full_time_teachers  || '',
