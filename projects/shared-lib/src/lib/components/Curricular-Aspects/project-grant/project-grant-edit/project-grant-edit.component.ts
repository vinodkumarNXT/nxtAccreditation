...
import { FormsService, FileUploadService, SwalService, MaterialModule, SharableModule } from 'shared-lib';

@Component({
  selector: 'lib-project-grant-edit',
  standalone: true,
  ...
  templateUrl: './project-grant-edit.component.html',
  styleUrl: './project-grant-edit.component.css'
})
export class ProjectGrantEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      fundingAgencyName: ['', Validators.required],
      type: ['', Validators.required],
      awardedYear: ['', Validators.required],
      departmentName: ['', Validators.required],
      fundsProvided: ['', Validators.required],
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        investigatorName: this.form.get("investigatorName")?.value,
        fundingAgencyName: this.form.get("fundingAgencyName")?.value,
        type: this.form.get("type")?.value,
        awardedYear: this.form.get("awardedYear")?.value,
        departmentName: this.form.get("departmentName")?.value,
        fundsProvided: this.form.get("fundsProvided")?.value,
        projectDuration: this.form.get("projectDuration")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.ProjectGrant);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.ProjectGrant);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          ...
          fundingAgencyName: formData?.funding_agency_name || '',
          awardedYear: formData?.awarded_year || '',
          fundsProvided: formData?.funds_provided || '',
          awardDuration: formData?.award_duration || '',
