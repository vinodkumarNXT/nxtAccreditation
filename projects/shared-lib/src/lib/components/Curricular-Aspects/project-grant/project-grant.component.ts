...
import { FileUploadService, FormsService, MaterialModule, ProjectGrantAddComponent, ProjectGrantEditComponent, ProjectGrantViewComponent, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-project-grant',
  standalone: true,
  ...
  templateUrl: './project-grant.component.html',
  styleUrl: './project-grant.component.css'
})
export class ProjectGrantComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "investigatorName", "fundingAgency","type","department" , "awardYear", "funds", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ProjectGrant);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(ProjectGrantAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(ProjectGrantViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(ProjectGrantEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.ProjectGrant;
