...
import { AwardedTeachersAddComponent, AwardedTeachersEditComponent, AwardedTeachersViewComponent,  FileUploadService, FormsService, MaterialModule, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-awarded-teachers',
  standalone: true,
  ...
  templateUrl: './awarded-teachers.component.html',
  styleUrl: './awarded-teachers.component.css'
})
export class AwardedTeachersComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "fullTimeTeachers", "totalTeachers","year","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AwardedTeachers);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(AwardedTeachersAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(AwardedTeachersViewComponent, {
...
//   const dialogRef = this.dialog.open(AwardedTeachersEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.AwardedTeachers;
