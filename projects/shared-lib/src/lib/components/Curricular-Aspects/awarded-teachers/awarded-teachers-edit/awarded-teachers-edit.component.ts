...
@Component({
  selector: 'lib-awarded-teachers-edit',
  standalone: true,
  ...
  templateUrl: './awarded-teachers-edit.component.html',
  styleUrl: './awarded-teachers-edit.component.css'
})
export class AwardedTeachersEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      fullTimeTeachers:["", Validators.required],
  ...
  */
  async updateForm(): Promise<void> {
    ...
    this.loading =true
     ...
     // Check if selectedFiles is defined and has files
     ...
     if (this.form.valid) {
      ...
      if (this.selectedFiles && this.selectedFiles.length > 0) {
        // If there are selected files, use them as is
        fileUploadData = this.selectedFiles;
      } else {
        // Convert previously uploaded files to Base64
        const base64Files = await Promise.all(
          (this.preLoadData.fileUpload || []).map(file => this.convertToBase64(file))
        );
        fileUploadData = base64Files; // Use Base64 converted files as fallback
      }
      
      
      const postData = {
        fullTimeTeachers: this.form.get('fullTimeTeachers')?.value,
        totalTeachers: this.form.get('totalTeachers')?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.AwardedTeachers);
  ...
  //   try {
  //     const response = await this.formsService.getFormData(this.data.id, endpoints.AwardedTeachers);
  ...
  //         fullTimeTeachers: formData?.full_time_teachers,
  //         totalTeachers: formData?.total_teachers,
  //         year: formData?.year,
  //         documentLink: formData?.document_links || '', // Ensure default value if 
    ...
    try {
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          ...
          totalTeachers: formData?.total_teachers,
          year: formData?.year,
          documentLink: formData?.document_links || '', // Ensure default value if null or empty
        });
  
        
        // Ensure fileUpload is not undefined or null, default to an empty array if so
        this.preLoadData = formData.upload_files; 
