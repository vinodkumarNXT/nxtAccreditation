...
@Component({
  selector: 'lib-biblio-metrics-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './biblio-metrics-view.component.html',
  styleUrl: './biblio-metrics-view.component.css'
})
export class BiblioMetricsViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.BiblioMetrics);
