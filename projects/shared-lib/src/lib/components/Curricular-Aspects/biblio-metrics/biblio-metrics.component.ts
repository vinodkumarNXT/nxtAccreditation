...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, EditedVolumesAddComponent, EditedVolumesEditComponent, EditedVolumesViewComponent, FileUploadService, SwalService, TablePaginationService, BiblioMetricsEditComponent, BiblioMetricsAddComponent, BiblioMetricsViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-biblio-metrics',
  standalone: true,
  ...
  templateUrl: './biblio-metrics.component.html',
  styleUrl: './biblio-metrics.component.css'
})
export class BiblioMetricsComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "paperTitle", "authorName", "journalName","publicationYear", "indexes", "actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.BiblioMetrics);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(BiblioMetricsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(BiblioMetricsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(BiblioMetricsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.BiblioMetrics;
