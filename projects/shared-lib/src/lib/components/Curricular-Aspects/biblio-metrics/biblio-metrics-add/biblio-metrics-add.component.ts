...
@Component({
  selector: 'lib-biblio-metrics-add',
  standalone: true,
  ...
  templateUrl: './biblio-metrics-add.component.html',
  styleUrl: './biblio-metrics-add.component.css'
})
export class BiblioMetricsAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.BiblioMetrics);
