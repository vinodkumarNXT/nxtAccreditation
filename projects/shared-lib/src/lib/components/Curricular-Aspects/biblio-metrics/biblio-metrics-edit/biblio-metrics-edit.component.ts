...
@Component({
  selector: 'lib-biblio-metrics-edit',
  standalone: true,
  ...
  templateUrl: './biblio-metrics-edit.component.html',
  styleUrl: './biblio-metrics-edit.component.css'
})
export class BiblioMetricsEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.BiblioMetrics);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.BiblioMetrics);
