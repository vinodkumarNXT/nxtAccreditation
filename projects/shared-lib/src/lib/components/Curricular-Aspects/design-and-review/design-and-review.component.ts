...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService,  DesignAndReviewAddComponent, DesignAndReviewViewComponent, DesignAndReviewEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-design-and-review',
  standalone: true,
  ...
  templateUrl: './design-and-review.component.html',
  styleUrl: './design-and-review.component.css'
})
export class DesignAndReviewComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "feedbackReceivedFrom", "year", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.DesignAndReview);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(DesignAndReviewAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(DesignAndReviewViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(DesignAndReviewEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.DesignAndReview;
