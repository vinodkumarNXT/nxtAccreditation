...
@Component({
  selector: 'lib-student-passpercentage-survey-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './student-passpercentage-survey-view.component.html',
  styleUrl: './student-passpercentage-survey-view.component.css'
})
export class StudentPasspercentageSurveyViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.StudentPassPercentageSurvey);
