...
@Component({
  ...
  templateUrl: './student-passpercentage-survey.component.html',
  styleUrl: './student-passpercentage-survey.component.css'
})
export class StudentPasspercentageSurveyComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "programeCode", "programName","studentsAppeared", "studentsPassed","actions"];

//   {
//     "id": 1,
//     "program_name": "Computer Science And Technology",
//     "programe_code": "CSE",
//     "students_appeared": 50,
//     "students_passed": 30,
//     "upload_files": [
//         "1725343562_Screenshot_(8).png"
//     ],
//     "document_links": "nvngjfgnfbd"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.StudentPassPercentageSurvey);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(StudentPasspercentageSurveyAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(StudentPasspercentageSurveyViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  // Open the dialog with the InstitutionIntegratesEditComponent
  const dialogRef = this.dialog.open(StudentPasspercentageSurveyEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.StudentPassPercentageSurvey;
