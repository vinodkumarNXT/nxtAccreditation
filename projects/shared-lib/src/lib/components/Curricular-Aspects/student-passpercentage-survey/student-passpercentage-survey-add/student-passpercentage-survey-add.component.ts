...
@Component({
  selector: 'lib-student-passpercentage-survey-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './student-passpercentage-survey-add.component.html',
  styleUrl: './student-passpercentage-survey-add.component.css'
})
export class StudentPasspercentageSurveyAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.StudentPassPercentageSurvey);
