...
@Component({
  selector: 'lib-gender-equity-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './gender-equity-add.component.html',
  styleUrl: './gender-equity-add.component.css'
})
export class GenderEquityAddComponent {
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, endpoints.GenderEquity);
