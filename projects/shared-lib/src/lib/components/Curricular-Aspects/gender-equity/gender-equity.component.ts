...
@Component({
  ...
  templateUrl: './gender-equity.component.html',
  styleUrl: './gender-equity.component.css'
})
export class GenderEquityComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "GenderEquity","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.GenderEquity);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(GenderEquityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(GenderEquityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(GenderEquityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.GenderEquity;
