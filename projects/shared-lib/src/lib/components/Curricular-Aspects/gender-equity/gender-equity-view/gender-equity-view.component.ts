...
@Component({
  selector: 'lib-gender-equity-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './gender-equity-view.component.html',
  styleUrl: './gender-equity-view.component.css'
})
export class GenderEquityViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.GenderEquity);
