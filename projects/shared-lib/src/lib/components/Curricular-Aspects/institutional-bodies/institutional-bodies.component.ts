...
@Component({
  ...
  templateUrl: './institutional-bodies.component.html',
  styleUrl: './institutional-bodies.component.css'
})
export class InstitutionalBodiesComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "institutionalBody","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.InstitutionalBodies);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(InstitutionalBodiesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(InstitutionalBodiesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(InstitutionalBodiesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.InstitutionalBodies;
