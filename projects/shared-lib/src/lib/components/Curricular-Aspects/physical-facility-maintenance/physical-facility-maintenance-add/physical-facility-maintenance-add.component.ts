...
@Component({
  selector: 'lib-physical-facility-maintenance-add',
  standalone: true,
  ...
  templateUrl: './physical-facility-maintenance-add.component.html',
  styleUrl: './physical-facility-maintenance-add.component.css'
})
export class PhysicalFacilityMaintenanceAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.PhysicalFacilityMaintenance);
