...
@Component({
  selector: 'lib-awards-received-add',
  standalone: true,
  ...
  templateUrl: './awards-received-add.component.html',
  styleUrl: './awards-received-add.component.css'
})
export class AwardsReceivedAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.AwardsReceived);
