...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, AwardsReceivedViewComponent, AwardsReceivedAddComponent, AwardsReceivedEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-awards-received',
  standalone: true,
  ...
  templateUrl: './awards-received.component.html',
  styleUrl: './awards-received.component.css'
})
export class AwardsReceivedComponent {
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AwardsReceived);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(AwardsReceivedAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(AwardsReceivedViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(AwardsReceivedEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.AwardsReceived;
