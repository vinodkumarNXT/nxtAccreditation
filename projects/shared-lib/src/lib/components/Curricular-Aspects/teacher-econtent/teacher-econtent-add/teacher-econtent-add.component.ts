...
@Component({
  selector: 'lib-teacher-econtent-add',
  standalone: true,
  ...
  templateUrl: './teacher-econtent-add.component.html',
  styleUrl: './teacher-econtent-add.component.css'
})
export class TeacherEcontentAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.TeacherEcontent);
