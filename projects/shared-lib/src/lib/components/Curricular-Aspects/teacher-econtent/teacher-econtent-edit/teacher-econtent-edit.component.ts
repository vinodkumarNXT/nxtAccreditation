...
@Component({
  selector: 'lib-teacher-econtent-edit',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './teacher-econtent-edit.component.html',
  styleUrl: './teacher-econtent-edit.component.css'
})
export class TeacherEcontentEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      platformCount:['', Validators.required],
      documentLink: [''],
    });
  }
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        ...
        platformName: this.form.get("platformName")?.value,
        date: this.form.get("date")?.value,
        platformCount: this.form.get("platformCount")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.TeacherEcontent);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.TeacherEcontent);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          ...
          platformName: formData?.platform_name || '',
          date: formData?.date || '',
          platformCount: formData?.no_of_platforms || '',
