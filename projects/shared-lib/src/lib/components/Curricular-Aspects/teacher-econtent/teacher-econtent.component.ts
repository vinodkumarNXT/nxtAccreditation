...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognizationsReceivedAddComponent, RecognizationsReceivedViewComponent, RecognizationsReceivedEditComponent, TeacherEcontentAddComponent, TeacherEcontentEditComponent, TeacherEcontentViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-teacher-econtent',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './teacher-econtent.component.html',
  styleUrl: './teacher-econtent.component.css'
})
export class TeacherEcontentComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "moduleName", "platformName", "platforms", "actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.TeacherEcontent);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(TeacherEcontentAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(TeacherEcontentViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(TeacherEcontentEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.TeacherEcontent;
