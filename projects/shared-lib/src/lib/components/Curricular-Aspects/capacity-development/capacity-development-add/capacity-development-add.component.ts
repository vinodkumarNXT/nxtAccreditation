...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, AutomationExaminationAddComponent, AutomationExaminationViewComponent, AutomationExaminationEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-capacity-development-add',
  standalone: true,
  ...
  templateUrl: './capacity-development-add.component.html',
  styleUrl: './capacity-development-add.component.css'
})
export class CapacityDevelopmentAddComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      agenciesConsultant: ["", Validators.required],
      documentLink: [''],
    });
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      const postData = {
        ...
        studentEnrolled: this.form.get("studentEnrolled").value,
        agenciesConsultant: this.form.get("agenciesConsultant").value,
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.CapacityDevelopment);
  ...
  async loadFormData() {
    try {
      ...
      const listData = await this.formsService.getListData(endpoints.CapacityDevelopment);
