...
export class CapacityDevelopmentComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "programName","date","studentEnrolled","agenciesConsultant","actions"];
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(CapacityDevelopmentAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(CapacityDevelopmentViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(CapacityDevelopmentEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.CapacityDevelopment;
