...
import { MaterialModule, SharableModule, FormsService, SnackbarService, SpinnerService, SwalService } from 'shared-lib';
import Swal from 'sweetalert2';



@Component({
  selector: 'lib-transferable-lifeskills-add',
  standalone: true,
  imports: [MaterialModule, SharableModule, NgxLoadingModule],
  providers: [
    FormsService,
    SnackbarService,
    provideNativeDateAdapter()
  ],
  templateUrl: './transferable-lifeskills-add.component.html',
  styleUrl: './transferable-lifeskills-add.component.css'
})
export class TransferableLifeskillsAddComponent {
  ...
  public listData: any;
  selectedFiles: { fileName: string; fileContent: string }[] = [];
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.TransferableLifeskills);
