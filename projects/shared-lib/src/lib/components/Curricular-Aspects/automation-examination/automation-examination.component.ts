...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, EndSemesterAddComponent, EndSemesterViewComponent, EndSemesterEditComponent, AutomationExaminationEditComponent, AutomationExaminationViewComponent, AutomationExaminationAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-automation-examination',
  standalone: true,
  ...
  templateUrl: './automation-examination.component.html',
  styleUrl: './automation-examination.component.css'
})
export class AutomationExaminationComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "automationText","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AutomationExamination);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(AutomationExaminationAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(AutomationExaminationViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(AutomationExaminationEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.AutomationExamination;
