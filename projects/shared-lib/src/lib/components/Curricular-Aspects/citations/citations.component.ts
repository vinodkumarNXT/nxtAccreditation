...
import { CitationsAddComponent, CitationsEditComponent, CitationsViewComponent, FileUploadService, FormsService, MaterialModule, SharableModule, SnackbarService, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-citations',
  standalone:true,
...
export class CitationsComponent {
  ...
  displayedColumns = ["id", "paperTitle", "authorName", "journalName","publicationYear", "H-indexes", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.CitationIndex);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(CitationsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(CitationsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(CitationsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.CitationIndex;
