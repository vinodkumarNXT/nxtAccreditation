...
@Component({
  ...
  templateUrl: './citations-edit.component.html',
  styleUrl: './citations-edit.component.css'
})
export class CitationsEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.CitationIndex);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.CitationIndex);
