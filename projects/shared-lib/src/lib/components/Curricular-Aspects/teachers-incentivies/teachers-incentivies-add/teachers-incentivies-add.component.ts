...
@Component({
  selector: 'lib-teachers-incentivies-add',
  standalone: true,
  ...
  templateUrl: './teachers-incentivies-add.component.html',
  styleUrl: './teachers-incentivies-add.component.css'
})
export class TeachersIncentiviesAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      const postData = {
        ...
        researchGuide: this.form.get("teacherName")?.value,
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.TeachersIncentivies);
