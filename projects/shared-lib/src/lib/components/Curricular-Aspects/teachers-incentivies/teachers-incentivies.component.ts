...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognizationsReceivedAddComponent, RecognizationsReceivedViewComponent, RecognizationsReceivedEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-teachers-incentivies',
  standalone: true,
  ...
  templateUrl: './teachers-incentivies.component.html',
  styleUrl: './teachers-incentivies.component.css'
})
export class TeachersIncentiviesComponent {



  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "qualification", "currentStatus", "scholarName","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.TeachersIncentivies);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(RecognizationsReceivedAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(RecognizationsReceivedViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(RecognizationsReceivedEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.TeachersIncentivies;
