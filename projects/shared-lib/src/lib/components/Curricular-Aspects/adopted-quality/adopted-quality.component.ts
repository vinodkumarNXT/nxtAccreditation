...
export class AdoptedQualityComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year","qualityEvents", "followUp","status","certification","actions"];
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(AdoptedQualityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(AdoptedQualityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(AdoptedQualityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.AdoptedQuality;
