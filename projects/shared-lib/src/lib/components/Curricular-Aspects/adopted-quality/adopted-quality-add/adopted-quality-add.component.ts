...
@Component({
  selector: 'lib-adopted-quality-add',
  standalone: true,
  ...
  templateUrl: './adopted-quality-add.component.html',
  styleUrl: './adopted-quality-add.component.css'
})
export class AdoptedQualityAddComponent {
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, endpoints.AdoptedQuality);
   ...
   updateForm() {
    ...
    async loadFormData() {
      try {
        ...
        const listData = await this.formsService.getListData(endpoints.AdoptedQuality);
