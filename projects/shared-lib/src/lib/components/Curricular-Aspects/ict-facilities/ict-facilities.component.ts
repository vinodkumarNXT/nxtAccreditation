...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, FunctionalMousAddComponent, FunctionalMousViewComponent, FunctionalMousEditComponent, ICTFacilitiesEditComponent, ICTFacilitiesViewComponent, ICTFacilitiesAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-ict-facilities',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './ict-facilities.component.html',
  styleUrl: './ict-facilities.component.css'
})
export class ICTFacilitiesComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "lcdCount", "wifiCount","seminarHallCount","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ICTFacilities);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ICTFacilitiesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ICTFacilitiesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ICTFacilitiesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ICTFacilities;
