...
import { FileUploadService, FormsService, MaterialModule, RecognitionAddComponent, RecognitionEditComponent, RecognitionViewComponent, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-recognition',
  standalone: true,
...
export class RecognitionComponent {
  ...
  displayedColumns = ["id", "departmentName", "schemeName", "fundingAgencyName", "fundsProvided", "actions"];
...
//     "department_name": "EEE",
//     "scheme_name": "test",
//     "funding_agency_name": "test",
//     "awarded_year": "2021",
//     "funds_provided": "20 lakhs",
//     "award_duration": "2 months",
//     "upload_files": [
//         ""
//     ],
//     "document_links": null
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.Recognition);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(RecognitionAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(RecognitionViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(RecognitionEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.Recognition;
