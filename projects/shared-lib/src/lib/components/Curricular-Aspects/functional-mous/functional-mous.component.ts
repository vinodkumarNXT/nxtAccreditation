...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ExtensionActivityAddComponent, ExtensionActivityViewComponent, ExtensionActivityEditComponent, FunctionalMousEditComponent, FunctionalMousViewComponent, FunctionalMousAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-functional-mous',
  standalone: true,
  ...
  templateUrl: './functional-mous.component.html',
  styleUrl: './functional-mous.component.css'
})
export class FunctionalMousComponent {



  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "organizationName", "instituteName","signingYear", "duration", "activities","actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.FunctionalMous);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(FunctionalMousAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(FunctionalMousViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(FunctionalMousEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.FunctionalMous;
