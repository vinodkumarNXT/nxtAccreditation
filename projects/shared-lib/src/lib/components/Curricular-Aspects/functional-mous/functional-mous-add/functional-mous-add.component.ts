...
@Component({
  selector: 'lib-functional-mous-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './functional-mous-add.component.html',
  styleUrl: './functional-mous-add.component.css'
})
export class FunctionalMousAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.FunctionalMous);
