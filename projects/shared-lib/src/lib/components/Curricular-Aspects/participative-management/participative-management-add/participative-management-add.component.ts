...
@Component({
  selector: 'lib-participative-management-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './participative-management-add.component.html',
  styleUrl: './participative-management-add.component.css'
})
export class ParticipativeManagementAddComponent {
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, endpoints.ParticipativeManagement);
