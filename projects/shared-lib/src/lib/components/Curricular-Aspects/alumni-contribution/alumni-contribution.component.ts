import { AlumniContributionEditComponent } from './alumni-contribution-edit/alumni-contribution-edit.component';
import { AlumniContributionAddComponent } from './alumni-contribution-add/alumni-contribution-add.component';
...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureViewComponent, InfrastractureExpenditureEditComponent, AlumniContributionViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-alumni-contribution',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './alumni-contribution.component.html',
  styleUrl: './alumni-contribution.component.css'
})
export class AlumniContributionComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year","alumniAssociation", "contribution","statement","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AlumniContribution);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(AlumniContributionAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(AlumniContributionViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(AlumniContributionEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.AlumniContribution;
