...
export class TeachingLearningFacilityComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teachingLearningText", "actions"];


//   {
//     "id": 2,
//     "teachingLearningText": "testing",
//     "uploadFiles": [
//         "1726466578_Screenshot_(8).png"
//     ],
//     "documentLinks": "test"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.TeachingLearningFacility);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(TeachingLearningFacilityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(TeachingLearningFacilityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(TeachingLearningFacilityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.TeachingLearningFacility;
