...
@Component({
  selector: 'lib-collaborative-activity-add',
  standalone: true,
  ...
  templateUrl: './collaborative-activity-add.component.html',
  styleUrl: './collaborative-activity-add.component.css'
})
export class CollaborativeActivityAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.CollaborativeActivity);
