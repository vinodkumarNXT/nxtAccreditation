...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, AwardsReceivedAddComponent, AwardsReceivedViewComponent, AwardsReceivedEditComponent, CollaborativeActivityEditComponent, CollaborativeActivityViewComponent, CollaborativeActivityAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-collaborative-activity',
  standalone: true,
  ...
  templateUrl: './collaborative-activity.component.html',
  styleUrl: './collaborative-activity.component.css'
})
export class CollaborativeActivityComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "activityTitle", "collaboratingAgency", "participantName","collabarationYear","duration",  "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.CollaborativeActivity);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(CollaborativeActivityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(CollaborativeActivityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(CollaborativeActivityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.CollaborativeActivity;
