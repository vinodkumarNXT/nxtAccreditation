...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, FinacialAuditEditComponent, FinacialAuditAddComponent, FinacialAuditViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-finacial-audit',
  standalone: true,
  ...
  templateUrl: './finacial-audit.component.html',
  styleUrl: './finacial-audit.component.css'
})
export class FinacialAuditComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "finacialAudit","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.FinacialAudit);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(FinacialAuditAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(FinacialAuditViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(FinacialAuditEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.FinacialAudit;
