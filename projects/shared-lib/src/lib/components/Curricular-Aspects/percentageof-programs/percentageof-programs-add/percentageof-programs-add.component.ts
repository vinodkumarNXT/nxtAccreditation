...
@Component({
  selector: 'lib-percentageof-programs-add',
  standalone: true,
  ...
  templateUrl: './percentageof-programs-add.component.html',
  styleUrl: './percentageof-programs-add.component.css'
})
export class PercentageofProgramsAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.CirAspPercentageProgrammes);
