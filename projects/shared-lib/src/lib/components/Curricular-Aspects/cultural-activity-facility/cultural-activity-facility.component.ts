import { CulturalActivityFacilityAddComponent } from './cultural-activity-facility-add/cultural-activity-facility-add.component';
...
export class CulturalActivityFacilityComponent {



  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "culturalFacilityText", "actions"];


//   {
//     "id": 1,
//     "culturalFacilityText": "testinggggggggggggggggggggggggg",
//     "uploadFiles": [
//         "1726469725_Screenshot_(8).png",
//         "1726469725_Screenshot_(8).png"
//     ],
//     "documentLinks": null
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.CulturalActivityFacility);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(CulturalActivityFacilityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(CulturalActivityFacilityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(CulturalActivityFacilityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.CulturalActivityFacility;
