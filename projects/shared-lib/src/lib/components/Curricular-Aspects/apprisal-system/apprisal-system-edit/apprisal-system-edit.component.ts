...
@Component({
  selector: 'lib-apprisal-system-edit',
  standalone: true,
  ...
  templateUrl: './apprisal-system-edit.component.html',
  styleUrl: './apprisal-system-edit.component.css'
})
export class ApprisalSystemEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        performance: this.form.get("performance")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.ApprisalSystem);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.ApprisalSystem);
