...
@Component({
  ...
  templateUrl: './non-govt-grants.component.html',
  styleUrl: './non-govt-grants.component.css'
})
export class NonGovtGrantsComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "year","fundingAgency", "grantPurpose","fundsReceived","auditStatement","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.NonGovtGrants);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(NonGovtGrantsAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(NonGovtGrantsViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(NonGovtGrantsEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.NonGovtGrants;
