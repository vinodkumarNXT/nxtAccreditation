...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, TeacherResearchAddComponent, TeacherResearchViewComponent, TeacherResearchEditComponent, ResearchFellowshipAddComponent, ResearchFellowshipViewComponent, ResearchFellowshipEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-research-fellowship',
  standalone: true,
  ...
  templateUrl: './research-fellowship.component.html',
  styleUrl: './research-fellowship.component.css'
})
export class ResearchFellowshipComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "award","awardYear","awardingAgency", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ResearchFellowship);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(ResearchFellowshipAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(ResearchFellowshipViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(ResearchFellowshipEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.ResearchFellowship;
