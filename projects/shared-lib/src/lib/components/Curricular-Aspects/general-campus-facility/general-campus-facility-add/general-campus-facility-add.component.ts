...
@Component({
  selector: 'lib-general-campus-facility-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './general-campus-facility-add.component.html',
  styleUrl: './general-campus-facility-add.component.css'
})
export class GeneralCampusFacilityAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.GeneralCampusFacility);
