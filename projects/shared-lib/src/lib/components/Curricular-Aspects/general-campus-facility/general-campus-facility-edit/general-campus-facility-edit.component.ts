...
@Component({
  selector: 'lib-general-campus-facility-edit',
  standalone: true,
  ...
  templateUrl: './general-campus-facility-edit.component.html',
  styleUrl: './general-campus-facility-edit.component.css'
})
export class GeneralCampusFacilityEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        ...
        timeTable: this.form.get("timeTable")?.value,
        fileUpload: this.selectedFiles
      };

      try {
        // Attempt to update the form data using the service
        await this.formsService.updateFormData(postData, this.editformID, endpoints.GeneralCampusFacility);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.GeneralCampusFacility);
