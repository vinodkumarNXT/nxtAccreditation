...
export class GeneralCampusFacilityComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "roomName","facilityType","timeTable", "actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.GeneralCampusFacility);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(GeneralCampusFacilityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(GeneralCampusFacilityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(GeneralCampusFacilityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.GeneralCampusFacility;
