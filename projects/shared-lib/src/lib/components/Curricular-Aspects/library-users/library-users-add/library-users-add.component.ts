...
@Component({
  selector: 'lib-library-users-add',
  standalone: true,
  ...
  templateUrl: './library-users-add.component.html',
  styleUrl: './library-users-add.component.css'
})
export class LibraryUsersAddComponent {
  ...
  // 'libraryResource' => 'required',
  //           'membership' => 'required',
  //           'eJournal' => 'required',
  //           'totalLibraryExpenditure' => 'required'    
  //           'documentLink'
  //           fileUpload
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.LibraryUsers);
  ...
  async loadFormData() {
    try {
      ...
      const listData = await this.formsService.getListData(endpoints.LibraryUsers);
