...
import { FileUploadService, FormsService, MaterialModule, SharableModule, StudentmentorRatioAddComponent, StudentmentorRatioEditComponent, StudentmentorRatioViewComponent, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-studentmentor-ratio',
  standalone: true,
  ...
  templateUrl: './studentmentor-ratio.component.html',
  styleUrl: './studentmentor-ratio.component.css'
})
export class StudentmentorRatioComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "noofTeachers","year", "actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.StudentsMentorRatio);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(StudentmentorRatioAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(StudentmentorRatioViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(StudentmentorRatioEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.StudentsMentorRatio;
