...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureViewComponent, InfrastractureExpenditureEditComponent, InstitutionalStatergicPlanAddComponent, InstitutionalStatergicPlanViewComponent, InstitutionIntegratesEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-institutional-statergic-plan',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './institutional-statergic-plan.component.html',
  styleUrl: './institutional-statergic-plan.component.css'
})
export class InstitutionalStatergicPlanComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "strategicPlan","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.InstitutionalStatergicPlan);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(InstitutionalStatergicPlanAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(InstitutionalStatergicPlanViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(InstitutionIntegratesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.InstitutionalStatergicPlan;
