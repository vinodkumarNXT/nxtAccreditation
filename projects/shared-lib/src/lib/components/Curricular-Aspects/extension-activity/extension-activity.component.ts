...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ExtensionActivityEditComponent, ExtensionActivityViewComponent, ExtensionActivityAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-extension-activity',
  standalone: true,
  ...
  templateUrl: './extension-activity.component.html',
  styleUrl: './extension-activity.component.css'
})
export class ExtensionActivityComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "socialIssue", "year",  "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ExtensionActivity);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ExtensionActivityAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ExtensionActivityViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ExtensionActivityEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ExtensionActivity;
