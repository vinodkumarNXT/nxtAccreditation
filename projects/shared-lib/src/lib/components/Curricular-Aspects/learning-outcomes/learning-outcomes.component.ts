...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, EndSemesterAddComponent, EndSemesterViewComponent, EndSemesterEditComponent, LearningOutcomesEditComponent, LearningOutcomesViewComponent, LearningOutcomesAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-learning-outcomes',
  standalone: true,
...
export class LearningOutcomesComponent {
  ...
  displayedColumns = ["id", "textForLearningOutcome", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.LearningOutcomes);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(LearningOutcomesAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(LearningOutcomesViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(LearningOutcomesEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.LearningOutcomes;
