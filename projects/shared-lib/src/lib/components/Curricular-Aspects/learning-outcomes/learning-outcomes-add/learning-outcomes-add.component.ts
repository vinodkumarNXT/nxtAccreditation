...
@Component({
  selector: 'lib-learning-outcomes-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './learning-outcomes-add.component.html',
  styleUrl: './learning-outcomes-add.component.css'
})
export class LearningOutcomesAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.LearningOutcomes);
