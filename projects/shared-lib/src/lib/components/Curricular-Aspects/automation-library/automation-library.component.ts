...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, CulturalActivityFacilityAddComponent, CulturalActivityFacilityViewComponent, CulturalActivityFacilityEditComponent, AutomationLibraryEditComponent, AutomationLibraryViewComponent, AutomationLibraryAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-automation-library',
  standalone: true,
  ...
  styleUrl: './automation-library.component.css'
})
export class AutomationLibraryComponent {
...
//   {
//     "id": 1,
//     "automationText": "testing",
//     "uploadFiles": [
//         "1726487278_Screenshot_(8).png"
//     ],
//     "documentLinks": "test1"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AutomationLibrary);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(AutomationLibraryAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(AutomationLibraryViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(AutomationLibraryEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.AutomationLibrary;
