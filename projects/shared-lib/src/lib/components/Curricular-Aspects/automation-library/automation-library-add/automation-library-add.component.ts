...
@Component({
  selector: 'lib-automation-library-add',
  standalone: true,
  ...
  templateUrl: './automation-library-add.component.html',
  styleUrl: './automation-library-add.component.css'
})
export class AutomationLibraryAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.AutomationLibrary);
