...
@Component({
  selector: 'lib-scholarship-schemes-edit',
  standalone: true,
  ...
  templateUrl: './scholarship-schemes-edit.component.html',
  styleUrl: './scholarship-schemes-edit.component.css'
})
export class ScholarshipSchemesEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.ScholarshipSchemes);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.ScholarshipSchemes);
