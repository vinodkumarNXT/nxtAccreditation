import { ScholarshipSchemesAddComponent } from './scholarship-schemes-add/scholarship-schemes-add.component';
...
export class ScholarshipSchemesComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "schemeName","studentCount","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ScholarshipSchemes);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ScholarshipSchemesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ScholarshipSchemesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ScholarshipSchemesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ScholarshipSchemes;
