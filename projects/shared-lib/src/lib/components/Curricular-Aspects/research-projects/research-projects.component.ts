...
import { FileUploadService, FormsService, MaterialModule, ResearchProjectsAddComponent, ResearchProjectsEditComponent, ResearchProjectsViewComponent, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-research-projects',
  standalone: true,
  ...
  styleUrl: './research-projects.component.css'
})
export class ResearchProjectsComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "qualification", "qualificationYear", "scholarName","currentStatus", "actions"];
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(ResearchProjectsAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(ResearchProjectsViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(ResearchProjectsEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.ResearchProjects;
