...
import { FileUploadService, FormsService, LearningProcessAddComponent, LearningProcessEditComponent, LearningProcessViewComponent, MaterialModule, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-learning-process',
  standalone: true,
  ...
  styleUrl: './learning-process.component.css'
})
export class LearningProcessComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "ictEnableTools", "actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.LearningProcess);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(LearningProcessAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(LearningProcessViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(LearningProcessEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.LearningProcess;
