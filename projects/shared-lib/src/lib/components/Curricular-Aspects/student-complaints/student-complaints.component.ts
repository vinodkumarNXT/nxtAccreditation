...
import { FileUploadService, FormsService, MaterialModule, SharableModule, StudentComplaintsAddComponent, StudentComplaintsEditComponent, StudentComplaintsViewComponent, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-student-complaints',
  standalone: true,
  ...
  styleUrl: './student-complaints.component.css'
})
export class StudentComplaintsComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "complaints", "Students","year", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.StudentComplaints);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(StudentComplaintsAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(StudentComplaintsViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(StudentComplaintsEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.StudentComplaints;
