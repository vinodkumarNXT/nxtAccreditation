...
@Component({
  selector: 'lib-student-complaints-edit',
  standalone: true,
  ...
  templateUrl: './student-complaints-edit.component.html',
  styleUrl: './student-complaints-edit.component.css'
})
export class StudentComplaintsEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      noOfComplaints: ['', Validators.required],
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        noOfComplaints: this.form.get("noOfComplaints")?.value,
        noOfStudents: this.form.get("noOfStudents")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.StudentComplaints);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.StudentComplaints);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          noOfComplaints: formData?.no_of_complaints || '',
          noOfStudents: formData?.no_of_students || '',
          year: formData?.year || '',
