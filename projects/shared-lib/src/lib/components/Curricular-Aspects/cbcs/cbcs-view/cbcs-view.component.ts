...
@Component({
  selector: 'app-cbcs-view',
  standalone:true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './cbcs-view.component.html',
  styleUrls: ['./cbcs-view.component.scss']
})
export class CbcsViewComponent implements OnInit {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.CBCS);
