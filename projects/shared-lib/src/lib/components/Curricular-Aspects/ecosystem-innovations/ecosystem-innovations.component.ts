...
import { EcosystemInnovationsAddComponent, EcosystemInnovationsEditComponent, EcosystemInnovationsViewComponent, FileUploadService, FormsService, MaterialModule, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-ecosystem-innovations',
  standalone: true,
  ...
  templateUrl: './ecosystem-innovations.component.html',
  styleUrl: './ecosystem-innovations.component.css'
})
export class EcosystemInnovationsComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "incubationCenterData", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.EcoSystemInnovations);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(EcosystemInnovationsAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(EcosystemInnovationsViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(EcosystemInnovationsEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.EcoSystemInnovations;
