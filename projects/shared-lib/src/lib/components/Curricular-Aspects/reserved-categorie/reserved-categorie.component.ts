...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ReservedCategoryAddComponent, ReservedCategoryViewComponent, ReservedCategoryEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-reserved-categorie',
  standalone: true,
  ...
  templateUrl: './reserved-categorie.component.html',
  styleUrl: './reserved-categorie.component.css'
})
export class ReservedCategorieComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "seatsReserved", "admitted", "year", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ReservedCategory);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(ReservedCategoryAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(ReservedCategoryViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(ReservedCategoryEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.ReservedCategory;
