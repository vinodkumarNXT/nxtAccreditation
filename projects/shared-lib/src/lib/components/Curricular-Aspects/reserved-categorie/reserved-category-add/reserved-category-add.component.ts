...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, SwalService, FileUploadService } from 'shared-lib';

@Component({
  selector: 'lib-reserved-category-add',
  standalone: true,
  ...
  templateUrl: './reserved-category-add.component.html',
  styleUrl: './reserved-category-add.component.css'
})
export class ReservedCategoryAddComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      noOfSeatsReserved: ['', Validators.required],
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      const postData = {
        ...
        noOfSeatsReserved: this.form.get('noOfSeatsReserved')?.value,
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.ReservedCategory);
