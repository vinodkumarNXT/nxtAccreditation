...
@Component({
  ...
  templateUrl: './internet-bandwidth.component.html',
  styleUrl: './internet-bandwidth.component.css'
})
export class InternetBandwidthComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "availableBandwidth","actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.InternetBandwidth);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(InternetBandwidthAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(InternetBandwidthViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(InternetBandwidthEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.InternetBandwidth;
