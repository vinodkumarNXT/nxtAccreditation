...
@Component({
  selector: 'lib-internet-bandwidth-add',
  standalone: true,
  ...
  templateUrl: './internet-bandwidth-add.component.html',
  styleUrl: './internet-bandwidth-add.component.css'
})
export class InternetBandwidthAddComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      availableBandwidth: ['', Validators.required],
      documentLink: [''],
    });
  }
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      const postData = {
        availableBandwidth: this.form.get("availableBandwidth")?.value,
        documentLink: this.form.get("documentLink")?.value,
        fileUpload: this.selectedFiles
      };

      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.InternetBandwidth);
