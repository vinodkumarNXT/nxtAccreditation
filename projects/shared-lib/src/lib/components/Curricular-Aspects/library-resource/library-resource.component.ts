...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, GeneralCampusFacilityAddComponent, GeneralCampusFacilityViewComponent, GeneralCampusFacilityEditComponent, LibraryResourceEditComponent, LibraryExpenditureViewComponent, LibraryResourceAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-library-resource',
  standalone: true,
  ...
  styleUrl: './library-resource.component.css'
})
export class LibraryResourceComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "libraryResource","membership","eJournal", "totalLibraryExpenditure","actions"];

//   {
//     "id": 1,
//     "libraryResource": "test",
//     "membership": "test1",
//     "eJournal": "test2",
//     "totalLibraryExpenditure": "10 L",
//     "uploadFiles": [
//         "1726548777_Screenshot_(8).png",
//         "1726548777_Screenshot_(8).png"
//     ],
//     "documentLinks": "test"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.LibraryResource);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(LibraryResourceAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(LibraryExpenditureViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(LibraryResourceEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.LibraryResource;
