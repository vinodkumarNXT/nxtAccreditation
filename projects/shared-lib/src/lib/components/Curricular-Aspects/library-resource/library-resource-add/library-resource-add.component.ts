...
@Component({
  selector: 'lib-library-resource-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './library-resource-add.component.html',
  styleUrl: './library-resource-add.component.css'
})
export class LibraryResourceAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.LibraryResource);
