...
@Component({
  selector: 'lib-library-resource-edit',
  standalone: true,
  ...
  templateUrl: './library-resource-edit.component.html',
  styleUrl: './library-resource-edit.component.css'
})
export class LibraryResourceEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.LibraryResource);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.LibraryResource);
