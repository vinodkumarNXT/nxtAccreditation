...
@Component({
  selector: 'lib-program-outcomes-add',
  standalone: true,
  ...
  templateUrl: './program-outcomes-add.component.html',
  styleUrl: './program-outcomes-add.component.css'
})
export class ProgramOutcomesAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.ProgramOutcomes);
