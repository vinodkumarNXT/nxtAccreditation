...
import { FileUploadService, FormsService, MaterialModule, SharableModule, SupportResearchAddComponent, SupportResearchEditComponent, SupportResearchViewComponent, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-support-research',
  standalone: true,
  ...
  templateUrl: './support-research.component.html',
  styleUrl: './support-research.component.css'
})
export class SupportResearchComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "supportResearch",  "actions"];
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(SupportResearchAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(SupportResearchViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(SupportResearchEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.SupportResearch;
