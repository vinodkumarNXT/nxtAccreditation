...
export class ExperimentalLearningComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "problemSolvingMethdology", "actions"];
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(ExperimentalLearningAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(ExperimentalLearningViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(ExperimentalLearningEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.ExperimentalLearning;
