import { FulltimeFacultyViewComponent } from './fulltime-faculty-view/fulltime-faculty-view.component';
...
export class FulltimeFacultyComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "designation","department","experience","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.FullTimeFaculty);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(FulltimeFacultyAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(FulltimeFacultyViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(FulltimeFacultyEditComponent, {
    // Pass the element's id as data to the dialog
    data: { id: val.id }
  });
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.FullTimeFaculty;
