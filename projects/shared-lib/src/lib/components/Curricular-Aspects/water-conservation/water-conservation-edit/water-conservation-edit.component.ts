...
@Component({
  selector: 'lib-water-conservation-edit',
  standalone: true,
  
    imports: [MaterialModule, SharableModule],
    providers: [FormsService, provideNativeDateAdapter()],
   
  templateUrl: './water-conservation-edit.component.html',
  styleUrl: './water-conservation-edit.component.css'
})
export class WaterConservationEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      waterConservation: ['', Validators.required],
  ...
  async loadMetaData() {
    try {
      ...
      const metaData = await this.formsService.getListData(endpoints.WaterConservation);
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        WaterConservation: this.form.get("waterConservation")?.value,
        documentLink: this.form.get("documentLink")?.value,
      };

      try {
        // Attempt to update the form data using the service
        await this.formsService.updateFormData(postData, this.editformID, endpoints.WaterConservation);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.WaterConservation);
      ...
      if (response && response.data) {
        ...
        // Patch the form with retrieved data'WaterConservation' => 'required'
              // fileUpload
        this.form.patchValue({
          waterConservation: formData?.WaterConservation || '',
