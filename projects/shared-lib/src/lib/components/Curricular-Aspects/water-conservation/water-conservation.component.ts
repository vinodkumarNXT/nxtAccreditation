...
import { FileUploadService, FormsService, MaterialModule, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, WaterConservationAddComponent, WaterConservationEditComponent, WaterConservationViewComponent, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-water-conservation',
  standalone: true,
  ...
  templateUrl: './water-conservation.component.html',
  styleUrl: './water-conservation.component.css'
})
export class WaterConservationComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "waterConservation","actions"];
//   {
//     "id": 1,
//     "WaterConservation": "test",
//     "upload_files": [
//         "1726832541_Screenshot_(8).png",
//         "1726832541_Screenshot_(8).png"
//     ],
//     "document_links": "test"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.WaterConservation);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(WaterConservationAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(WaterConservationViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(WaterConservationEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.WaterConservation;
