...
@Component({
  selector: 'lib-green-campus-edit',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  providers: [FormsService,provideNativeDateAdapter()],
  templateUrl: './green-campus-edit.component.html',
  styleUrl: './green-campus-edit.component.css'
})

export class GreenCampusEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      GreenCampus: ['', Validators.required],
    });
  }
  ...
  async loadMetaData() {
    try {
      ...
      const metaData = await this.formsService.getListData(endpoints.GreenCampus);
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        GreenCampus: this.form.get("GreenCampus")?.value,
      };

      try {
        // Attempt to update the form data using the service
        await this.formsService.updateFormData(postData, this.editformID, endpoints.GreenCampus);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.GreenCampus);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          GreenCampus: formData?.GreenCampus || '',
