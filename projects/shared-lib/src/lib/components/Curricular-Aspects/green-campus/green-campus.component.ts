...
import { UnderscoreToSpacePipe, YearMonthDayPipe, FileUploadService, TablePaginationService, GreenCampusAddComponent, GreenCampusViewComponent, GreenCampusEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-green-campus',
  standalone: true,
  ...
  templateUrl: './green-campus.component.html',
  styleUrl: './green-campus.component.css'
})
export class GreenCampusComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "greenCampus","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.GreenCampus);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(GreenCampusAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(GreenCampusViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(GreenCampusEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.GreenCampus;
