...
@Component({
  selector: 'lib-green-campus-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './green-campus-view.component.html',
  styleUrl: './green-campus-view.component.css'
})
export class GreenCampusViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.GreenCampus);
