...
@Component({
  ...
  templateUrl: './library-expenditure.component.html',
  styleUrl: './library-expenditure.component.css'
})
export class LibraryExpenditureComponent {
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.LibraryExpenditure);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(LibraryExpenditureAddComponent, {});
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(LibraryExpenditureEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.LibraryExpenditure;
