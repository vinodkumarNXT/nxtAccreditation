...
@Component({
  selector: 'lib-library-expenditure-add',
  standalone: true,
  ...
  templateUrl: './library-expenditure-add.component.html',
  styleUrl: './library-expenditure-add.component.css'
})
export class LibraryExpenditureAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.LibraryExpenditure);
