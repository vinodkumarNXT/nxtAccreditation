...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService,  ActivityPercentageEditComponent, ActivityPercentageViewComponent, ActivityPercentageAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-activity-percentage',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './activity-percentage.component.html',
  styleUrl: './activity-percentage.component.css'
})
export class ActivityPercentageComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "activityName", "awardName", "awardBody","awardYear",  "actions"];
...
// ]
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.ActivityPercentage);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(ActivityPercentageAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(ActivityPercentageViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(ActivityPercentageEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.ActivityPercentage;
