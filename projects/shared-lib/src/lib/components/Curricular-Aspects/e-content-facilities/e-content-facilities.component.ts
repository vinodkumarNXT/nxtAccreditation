...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ICTFacilitiesAddComponent, ICTFacilitiesViewComponent, ICTFacilitiesEditComponent, EContentFacilitiesViewComponent, EContentFacilitiesAddComponent, EContentFacilitiesEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-e-content-facilities',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './e-content-facilities.component.html',
  styleUrl: './e-content-facilities.component.css'
})
export class EContentFacilitiesComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "facilityName","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.EcontentFacilities);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(EContentFacilitiesAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(EContentFacilitiesViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(EContentFacilitiesEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.EcontentFacilities;
