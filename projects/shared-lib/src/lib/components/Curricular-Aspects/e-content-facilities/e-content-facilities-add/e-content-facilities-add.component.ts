...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, ImageService } from 'shared-lib';

@Component({
  selector: 'lib-e-content-facilities-add',
  standalone: true,
  ...
  templateUrl: './e-content-facilities-add.component.html',
  styleUrl: './e-content-facilities-add.component.css'
})
export class EContentFacilitiesAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      const postData = {
        facilityName: this.form.get("facilityName")?.value,
        documentLink: this.form.get("documentLink")?.value,
        fileUpload: this.selectedFiles
      };

      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.EcontentFacilities);
