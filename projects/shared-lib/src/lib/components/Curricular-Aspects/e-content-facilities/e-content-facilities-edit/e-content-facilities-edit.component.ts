...
@Component({
  selector: 'lib-e-content-facilities-edit',
  standalone: true,
  ...
  templateUrl: './e-content-facilities-edit.component.html',
  styleUrl: './e-content-facilities-edit.component.css'
})
export class EContentFacilitiesEditComponent {
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.EcontentFacilities);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.EcontentFacilities);
