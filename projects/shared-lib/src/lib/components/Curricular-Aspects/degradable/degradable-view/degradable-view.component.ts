...
@Component({
  selector: 'lib-degradable-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './degradable-view.component.html',
  styleUrl: './degradable-view.component.css'
})
export class DegradableViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.Degradable);
