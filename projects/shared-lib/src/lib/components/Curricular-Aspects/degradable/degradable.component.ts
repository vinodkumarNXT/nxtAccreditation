...
export class DegradableComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "Degradable","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.Degradable);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(DegradableAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(DegradableViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(DegradableEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.Degradable;
