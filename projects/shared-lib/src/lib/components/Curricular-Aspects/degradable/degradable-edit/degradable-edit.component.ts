...
@Component({
  ...
  templateUrl: './degradable-edit.component.html',
  styleUrl: './degradable-edit.component.css'
})
export class DegradableEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      degradable: ['', Validators.required],
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        Degradable: this.form.get("degradable")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.Degradable);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.Degradable);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          degradable: formData?.Degradable || '',
