...
import { FeedbackProcessAddComponent, FeedbackProcessEditComponent, FeedbackProcessViewComponent, FileUploadService, FormsService, MaterialModule, SharableModule, SwalService, TablePaginationService, UnderscoreToSpacePipe, YearMonthDayPipe } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-feedback-process',
  standalone: true,
  ...
  templateUrl: './feedback-process.component.html',
  styleUrl: './feedback-process.component.css'
})
export class FeedbackProcessComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "feedbackProcessed", "year", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.FeedbackProceess);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(FeedbackProcessAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(FeedbackProcessViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(FeedbackProcessEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.FeedbackProceess;
