import { StudentmentorRatioAddComponent } from './../studentmentor-ratio/studentmentor-ratio-add/studentmentor-ratio-add.component';
...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, ITFacilitiesAddComponent, ITFacilitiesViewComponent, ITFacilitiesEditComponent, StudentComplaintsEditComponent, StudentComplaintsViewComponent, StudentComplaintsAddComponent, StudentComputerRatioEditComponent, StudentComputerRatioViewComponent, StudentComputerRatioAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-student-computer-ratio',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './student-computer-ratio.component.html',
  styleUrl: './student-computer-ratio.component.css'
})
export class StudentComputerRatioComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "computerCount","studentCount","actions"];
...
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.StudentComputerRatio);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(StudentComputerRatioAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(StudentComputerRatioViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(StudentComputerRatioEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.StudentComputerRatio;
