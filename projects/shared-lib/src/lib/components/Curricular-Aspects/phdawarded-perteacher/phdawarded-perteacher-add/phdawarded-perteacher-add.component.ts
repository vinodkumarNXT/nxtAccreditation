...
@Component({
  selector: 'lib-phdawarded-perteacher-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './phdawarded-perteacher-add.component.html',
  styleUrl: './phdawarded-perteacher-add.component.css'
})
export class PhdawardedPerteacherAddComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      phdScholarName: ['', Validators.required],
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      const postData = {
        phdScholarName: this.form.get("phdScholarName")?.value,
        awardYear: this.form.get("awardYear")?.value,
        departmentName: this.form.get("departmentName")?.value,
        guideName: this.form.get("guideName")?.value,
        registrationYear: this.form.get("registrationYear")?.value,
        documentLink: this.form.get("documentLink")?.value,
        fileUpload: this.selectedFiles
      };

      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.PhdAwardedPerTeacher);
  ...
  async loadFormData() {
    try {
      ...
      const listData = await this.formsService.getListData(endpoints.PhdAwardedPerTeacher);
