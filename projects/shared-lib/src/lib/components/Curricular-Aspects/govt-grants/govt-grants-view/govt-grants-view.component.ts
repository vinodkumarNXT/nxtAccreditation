...
@Component({
  selector: 'lib-govt-grants-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './govt-grants-view.component.html',
  styleUrl: './govt-grants-view.component.css'
})
export class GovtGrantsViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.GovtGrants);
