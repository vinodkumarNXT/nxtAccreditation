...
import { FormsService, FileUploadService, SwalService, SharableModule, MaterialModule } from 'shared-lib';

@Component({
  selector: 'lib-patent-published-edit',
  standalone: true,
  ...
  templateUrl: './patent-published-edit.component.html',
  styleUrl: './patent-published-edit.component.css'
})
export class PatentPublishedEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      articalLink: ['', Validators.required],
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        ...
        articalLink: this.form.get("articalLink")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.PatentPublished);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.PatentPublished);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
          ...
          website: formData?.journal_website || '',
          articalLink: formData?.artical_link || '',
          index: formData?.indexes || '',
