...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognizationsReceivedAddComponent, RecognizationsReceivedViewComponent, RecognizationsReceivedEditComponent, PatentPublishedEditComponent, PatentPublishedViewComponent, PatentPublishedAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-patent-published',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './patent-published.component.html',
  styleUrl: './patent-published.component.css'
})
export class PatentPublishedComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "paperTitle", "authorName", "departmentName", "journalName","issnNumber","actions"];
...
//     "issn_number": "12345",
//     "journal_website": "wwww.test.com",
//     "artical_link": "wwww.test.com",
//     "indexes": "5",
//     "upload_files": [
//         "1725945458_Screenshot_(8).png"
//     ],
//     "document_links": "test"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.PatentPublished);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(PatentPublishedAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(PatentPublishedViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(PatentPublishedEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.PatentPublished;
