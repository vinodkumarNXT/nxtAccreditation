...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, StudentmentorRatioAddComponent, StudentmentorRatioViewComponent, StudentmentorRatioEditComponent, PlacementPercentageAddComponent, PlacementPercentageViewComponent, PlacementPercentageEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-placement-percentage',
  standalone: true,
  ...
  templateUrl: './placement-percentage.component.html',
  styleUrl: './placement-percentage.component.css'
})
export class PlacementPercentageComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id","year",  "passedoutStudentCount","placedStudentcount","placedCompany","actions"];

//   {
//     "id": 1,
//     "year": "2021",
//     "passedoutStudentCount": 200,
//     "placedStudentcount": 150,
//     "placedCompany": "test",
//     "uploadFiles": [
//         "1726745277_Screenshot_(8).png",
//         "1726745277_Screenshot_(8).png"
//     ],
//     "documentLinks": null
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.PlacementPercentage);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(PlacementPercentageAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(PlacementPercentageViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(PlacementPercentageEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.PlacementPercentage;
