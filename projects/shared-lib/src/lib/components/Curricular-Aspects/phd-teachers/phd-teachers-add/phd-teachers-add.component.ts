...
@Component({
  ...
  templateUrl: './phd-teachers-add.component.html',
  styleUrl: './phd-teachers-add.component.css'
})
export class PhdTeachersAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.PhdTeachers);
  ...
  async loadFormData() {
    try {
      ...
      const listData = await this.formsService.getListData(endpoints.PhdTeachers);
