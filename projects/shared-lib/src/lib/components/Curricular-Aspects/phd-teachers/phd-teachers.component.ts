...
export class PhdTeachersComponent {
  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "qualification","researchGuide","guideRecognitionYear", "scholarName", "actions"];
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(PhdTeachersAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(PhdTeachersViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(PhdTeachersEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.PhdTeachers;
