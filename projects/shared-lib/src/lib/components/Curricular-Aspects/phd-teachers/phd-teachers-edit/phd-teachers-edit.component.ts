import { FileUploadService, FormsService, MaterialModule, SharableModule, SwalService } from 'shared-lib';
...
@Component({
  selector: 'lib-phd-teachers-edit',
  standalone: true,
  ...
  templateUrl: './phd-teachers-edit.component.html',
  styleUrl: './phd-teachers-edit.component.css'
})
export class PhdTeachersEditComponent {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      guideRecognitionYear: ['', Validators.required],
      currentstatus: ['', Validators.required],
      scholarName: ['', Validators.required],
      guideLetterLink: ['', Validators.required],
      scholarRegistrationYear: ['', Validators.required],
      documentLink: [''],
    });
  }
  ...
  */
  async updateForm(): Promise<void> {
    ...
    if (this.form.valid) {
      const postData = {
        ...
        qualificationYear:this.form.get("qualificationYear")?.value,
        researchGuide:this.form.get("researchGuide")?.value,
        guideRecognitionYear:this.form.get("guideRecognitionYear")?.value,
        currentstatus:this.form.get("currentstatus")?.value,
        scholarName:this.form.get("scholarName")?.value,
        guideLetterLink:this.form.get("guideLetterLink")?.value,
        scholarRegistrationYear:this.form.get("scholarRegistrationYear")?.value,
      ...
      try {
        ...
        await this.formsService.updateFormData(postData, this.editformID, endpoints.PhdTeachers);
  ...
  async loadFormData(): Promise<void> {
    try {
      const response = await this.formsService.getFormData(this.data.id, endpoints.PhdTeachers);
      ...
      if (response && response.data) {
        ...
        this.form.patchValue({
            ...
            qualificationYear:formData?.qualification_year ,
            researchGuide:formData?.research_guide ,
            guideRecognitionYear:formData?.guide_recognition_year ,
            currentstatus:formData?.current_status ,
            scholarName:formData?.scholar_name ,
            scholarRegistrationYear:formData?.scholar_registration_year ,
            guideLetterLink:formData.guide_letter_link,
