import { AlternateEnergySourceEditComponent } from './alternate-energy-source-edit/alternate-energy-source-edit.component';
...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, InfrastractureExpenditureAddComponent, InfrastractureExpenditureViewComponent, InfrastractureExpenditureEditComponent, AlternateEnergySourceAddComponent, AlternateEnergySourceViewComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-alternate-energy-source',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './alternate-energy-source.component.html',
  styleUrl: './alternate-energy-source.component.css'
})
export class AlternateEnergySourceComponent {


  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "AlternateSource","actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.AlternateEnergySource);
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(AlternateEnergySourceAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(AlternateEnergySourceViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(AlternateEnergySourceEditComponent, {
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.AlternateEnergySource;
