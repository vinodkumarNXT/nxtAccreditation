...
@Component({
  selector: 'lib-alternate-energy-source-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './alternate-energy-source-add.component.html',
  styleUrl: './alternate-energy-source-add.component.css'
})
export class AlternateEnergySourceAddComponent {
    ...
    async submitForm() {
      ...
      if (this.form.valid) {
        ...
        try {
          const response = await this.formsService.onFormSubmit(postData, endpoints.AlternateEnergySource);
