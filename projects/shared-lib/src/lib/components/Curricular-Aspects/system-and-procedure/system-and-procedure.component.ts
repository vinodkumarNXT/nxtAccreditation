...
export class SystemAndProcedureComponent {

  pageSizeOptions: number[] = [];

  displayedColumns = ["id", "detailsText", 'actions'];
  ...
  openAddDialog(): void {
    // Open the dialog 
    const dialogRef = this.dialog.open(SystemAndProcedureAddComponent, {});
  ...
  viewElement(val: any): void {
    ...
    const dialogRef = this.dialog.open(SystemAndProcedureViewComponent, {
  ...
  async editElement(val: any): Promise<void> {
    ...
    const dialogRef = this.dialog.open(SystemAndProcedureEditComponent, {
      // Pass the element's id as data to the dialog
      data: { id: val.id }
    });
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.SystemAndProcedure;
