...
import { MaterialModule, SharableModule, FormsService, SnackbarService, SwalService } from 'shared-lib';

@Component({
  selector: 'lib-system-and-procedure-add',
  standalone: true,
  ...
  templateUrl: './system-and-procedure-add.component.html',
  styleUrl: './system-and-procedure-add.component.css'
})
export class SystemAndProcedureAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.SystemAndProcedure);
...
async loadFormData() {
  try {
    ...
    const listData = await this.formsService.getListData(endpoints.SystemAndProcedure);
