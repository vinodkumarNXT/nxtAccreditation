...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, SwalService, TablePaginationService, DemandRatioAddComponent, DemandRatioViewComponent, DemandRatioEditComponent, FileUploadService } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-demand-ratio',
  standalone: true,
  ...
  templateUrl: './demand-ratio.component.html',
  styleUrl: './demand-ratio.component.css'
})
export class DemandRatioComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "programmeName", "programmeCode", "sanctionedSeats", "admitted", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.DemandRatio);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(DemandRatioAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(DemandRatioViewComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.DemandRatio;
