...
@Component({
  selector: 'lib-demand-ratio-view',
  standalone: true,
  imports: [MaterialModule, SharableModule],
  templateUrl: './demand-ratio-view.component.html',
  styleUrl: './demand-ratio-view.component.css'
})
export class DemandRatioViewComponent {
 ...
 // Implementation  Load Form  Start Here
   async loadFormData() {
     try {
       const formData = await this.formsService.getFormData(this.data.id, endpoints.DemandRatio);
