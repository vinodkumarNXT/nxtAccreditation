...
@Component({
  selector: 'lib-teacher-research-add',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe, MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './teacher-research-add.component.html',
  styleUrl: './teacher-research-add.component.css'
})
export class TeacherResearchAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.TeacherResearch);
