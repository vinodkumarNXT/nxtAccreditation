...
@Component({
  ...
  styleUrl: './teacher-research.component.css'
})
export class TeacherResearchComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "teacherName", "seedMoney","year", "actions"];
...
//     "seed_money": "200000",
//     "year": "2021",
//     "policy_document_link": "www.link.com",
//     "upload_files": [
//         ""
//     ],
//     "document_links": "test"
// }
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.TeacherResearch);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(TeacherResearchAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(TeacherResearchViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(TeacherResearchEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.TeacherResearch;
