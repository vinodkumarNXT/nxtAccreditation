...
@Component({
  selector: 'lib-funding-research-add',
  standalone: true,
  ...
  templateUrl: './funding-research-add.component.html',
  styleUrl: './funding-research-add.component.css'
})
export class FundingResearchAddComponent {
  ...
  async submitForm() {
    ...
    if (this.form.valid) {
      ...
      try {
        const response = await this.formsService.onFormSubmit(postData, endpoints.FundingResearch);
