...
import { MaterialModule, SharableModule,  YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognitionAddComponent, RecognitionViewComponent, RecognitionEditComponent, FundingResearchViewComponent, FundingResearchAddComponent, FundingResearchEditComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-funding-research',
  standalone: true,
  imports: [MaterialModule, SharableModule,  YearMonthDayPipe,MatSort],
  providers: [FormsService, provideNativeDateAdapter()],
  templateUrl: './funding-research.component.html',
  styleUrl: './funding-research.component.css'
})
export class FundingResearchComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "investigatorName", "fundingAgency", "department", "awardYear", "fund", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.FundingResearch);
...
openAddDialog(): void {
  // Open the dialog 
  const dialogRef = this.dialog.open(FundingResearchAddComponent, {});
...
viewElement(val: any): void {
  ...
  const dialogRef = this.dialog.open(FundingResearchViewComponent, {
...
async editElement(val: any): Promise<void> {
  ...
  const dialogRef = this.dialog.open(FundingResearchEditComponent, {
...
async deleteElement(val: any): Promise<void> {
  const id = val.id;
  const endpoint = endpoints.FundingResearch;
