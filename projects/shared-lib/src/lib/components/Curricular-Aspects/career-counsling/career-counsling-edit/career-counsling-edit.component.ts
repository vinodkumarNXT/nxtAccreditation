...
import { MaterialModule, SharableModule, FormsService, SnackbarService, SpinnerService, FileUploadService, SwalService } from 'shared-lib';

@Component({
  selector: 'lib-career-counsling-edit',
  standalone: true,
  ...
  templateUrl: './career-counsling-edit.component.html',
  styleUrl: './career-counsling-edit.component.css'
})
export class CareerCounslingEditComponent {
...
async updateForm(): Promise<void> {
  ...
  if (this.form.valid) {
    ...
    try {
      ...
      await this.formsService.updateFormData(postData, this.editformID, endpoints.DesignAndReview);
...
async loadFormData(): Promise<void> {
  try {
    const response = await this.formsService.getFormData(this.data.id, endpoints.CareerCounseling);
    ...
    if (response && response.data) {
      ...
      this.form.patchValue({
       schemeName: formData.schemeName,
        govtStudent: formData.govtStudent,
        govtStudentAmount: formData.govtStudentAmount,
        institutionStudents: formData.institutionStudents,
        institutionStudentsAmount: formData.institutionStudentsAmount,
        ngoStudent: formData.ngoStudent,
        ngoStudentAmount: formData.ngoStudentAmount,
        ngoName: formData.ngoStudentAmount,
        Year: formData.Year,
        documentLink: formData.documentLinks || '', // Ensure default value if 
      });
