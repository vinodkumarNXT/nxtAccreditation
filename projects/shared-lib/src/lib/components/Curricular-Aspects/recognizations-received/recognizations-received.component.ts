...
import { MaterialModule, SharableModule, UnderscoreToSpacePipe, YearMonthDayPipe, FormsService, FileUploadService, SwalService, TablePaginationService, RecognitionAddComponent, RecognitionViewComponent, RecognitionEditComponent, RecognizationsReceivedEditComponent, RecognizationsReceivedViewComponent, RecognizationsReceivedAddComponent } from 'shared-lib';
import Swal from 'sweetalert2';

@Component({
  selector: 'lib-recognizations-received',
  standalone: true,
  ...
  templateUrl: './recognizations-received.component.html',
  styleUrl: './recognizations-received.component.css'
})
export class RecognizationsReceivedComponent {

  pageSizeOptions: number[] = [];
  displayedColumns = ["id", "AwardName", "awardAgencyName", "year", "actions"];
  ...
  async loadFormData() {
    try {
      const listData = await this.formsService.getListData(endpoints.RecognizationsReceived);
  ...
  async deleteElement(val: any): Promise<void> {
    const id = val.id;
    const endpoint = endpoints.RecognizationsReceived;
